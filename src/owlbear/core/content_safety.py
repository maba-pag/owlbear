...
def wrap_untrusted_content(text: str, *, source_url: str | None = None) -> str:
    ...
    # The advisory already contains '<untrusted_web_content>' which serves as
    # the opening tag when no URL is needed.  When a source_url is provided we
    # emit an explicit opening tag with the url attribute.
    if source_url is not None:
        open_tag = f'<untrusted_web_content url="{source_url}">'
        return f"{_ADVISORY}\n{open_tag}\n{text}\n{_CLOSE_TAG}"

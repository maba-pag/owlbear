...
Follows the same pattern as :class:`~owlbear.memory.knowledge.graph.GraphStore`:
constructor takes an open ``sqlite3.Connection``, JSON helpers for the
``config`` dict, and simple parameterized queries.
"""
...
class KnowledgeSourceStore:
    ...
    def create(self, source: KnowledgeSource) -> None:
        ...
        Raises :class:`sqlite3.IntegrityError` on duplicate ``(name, scope)``.
        """
        self._conn.execute(
    ...
    def list_enabled(self, scope: str | None = None) -> list[KnowledgeSource]:
        """Return enabled sources ordered by priority DESC.

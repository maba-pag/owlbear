...
| `main` | Consumer-facing branch — auto-synced product subset | Product files only: `share/`, `serve/`, `seed/`, `setup/`, `pyproject.toml`, `uv.lock`, `README.md`, `.gitignore`, `.gitattributes`, `.python-version`, `SECURITY.md`, `.editorconfig`, `.markdownlint-cli2.jsonc`, `.markdownlint.json`, `.markdownlintignore`, `.yamllint.yml` |
...
The consumer branch intentionally excludes development-only automation, workspace state, test
suites, and root tooling. The exact boundary is defined by `.github/sync-manifest.json`; those
paths remain on `dev` and are not part of the generated product surface.

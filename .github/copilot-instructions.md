...
| `main` | Consumer-facing branch — auto-synced product subset | Product files plus `.github/workflows/`: `share/`, `serve/`, `seed/`, `setup/`, `pyproject.toml`, `uv.lock`, `README.md`, `.gitignore`, `.gitattributes`, `.python-version`, `SECURITY.md`, `.editorconfig`, `.markdownlint-cli2.jsonc`, `.markdownlint.json`, `.markdownlintignore`, `.yamllint.yml` |
...
The consumer branch retains `.github/workflows/` so GitHub can discover and execute repository
automation, while excluding development-only automation support, workspace state, test suites, and
root tooling. The exact boundary is defined by `.github/sync-manifest.json`; excluded paths remain
on `dev` and are not part of the generated product surface.

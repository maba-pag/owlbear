...
| Runtime         | Standalone daemon                          | BearClaw CLI daemon; dual-coroutine (`channel_loop` + `poll_loop`) in `asyncio.TaskGroup`; `HookWorkerSupervisor` (`owlbear.core.hook_worker_supervisor`) owns background-task set + semaphore-bounded concurrency for hook-triggered work (wired in #966/#953); replaces bare `asyncio.create_task()` calls in `RetrospectiveHook`; bootstrap appends `supervisor.shutdown` to `BootstrapResult.cleanup` when ingest pipeline is available; `HookReactionRouter` (`owlbear.core.hook_reaction_router`) routes hook events to injected executors (notify/retry/escalate) based on event name and shallow scalar match predicates; wired in `build_hooks()` via #955; real executors from #956 (retry) and #957 (notify/escalate) |

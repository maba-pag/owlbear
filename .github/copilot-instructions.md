...
| HTTP            | httpx + truststore                         | Async HTTP client with system certificate trust; every `httpx.AsyncClient` must have an explicit `timeout=httpx.Timeout(T, connect=5)` where T varies by use case (see `docs/httpx-timeout-research.md`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Messaging       | Slack (slack_sdk)                          | Socket Mode WebSocket + AsyncWebClient (`RateLimitErrorRetryHandler(max_retry_count=1)` enabled); see `bearclaw slack` CLI; `slack_templates.py` Block Kit builders (proposal, status, interactive proposal, approval, progress) + text fallbacks; interactive handler routes `block_actions` to message queue or registered `register_action` callbacks by `action_id`; thread registry auto-threads messages by `context_key`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Safety          | Approval gates                             | `ApprovalPolicy` rules in config gate destructive tools (git push, PRs, deploy, run_command); `ApprovalGateToolset` wraps toolsets in bootstrap (sends Block Kit approval buttons on Slack, plain text on CLI); per-session pre-grants; `TerminalToolset.run_command()` confines `working_dir` to workspace root (null-byte rejection, resolve + `is_relative_to` check, `PermissionError` on escape); `ErrorJournal` append-only JSONL log in `.owlbear/error_journal.jsonl` with 10K-entry rotation + `query` for agent learning; `EscalationHook` wires ON_ERROR to user prompt (retry/skip/abort) when retries exhausted; `error_to_user_message()` in `owlbear.core.errors` sanitizes all channel-facing exception strings (type-based safe messages + regex scrub of tokens/secrets)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `done`        | Verified complete, ready for archival                  | Auditor    | AC verified with evidence, confidence ≥ .80, archived       |
...
| Auditor    | `done` → archived        | Verifies AC with evidence, scores confidence, archives confirmed tasks, commits + pushes.      |
...
| Auditor   | `done` → `review`      | Verification failed — evidence doesn't match AC    |
| Auditor   | `done` → `backlog`     | Fundamental quality issue — needs re-design        |
...
11. Close:          auditor verifies AC, archives, commits, pushes
12. Phase gate:     coverage >= 90%, all tests green, ruff clean
```

### Agent inventory

| Agent          | Role                                                           | user-invocable | Pattern                                         |
| -------------- | -------------------------------------------------------------- | -------------- | ----------------------------------------------- |
...
| auditor        | Verify done tasks, archive confirmed, commit + push            | Yes            | Gate: done → archived, uses task-verification |
...
| kanban-based-development | Autonomous single-workspace dev workflow — claims, handoff, defer-to-user protocol                                                                                                |
...
| task-verification        | Exit gate verification: verify AC with evidence → score confidence → archive or reject → commit. Used by auditor agent.                                                       |
...
| `.github/instructions/agent-common.instructions.md`  | `.github/agents/**` | Shared agent rules — task discipline, evidence              |
...
| `.github/prompts/commit-and-archive.prompt.md` | auditor      | Verify and archive done tasks, commit + push in packages |

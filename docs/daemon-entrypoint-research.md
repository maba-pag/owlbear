...
Task #124 requires three CLI commands (`bearclaw run`, `stop`, `status`) and daemon lifecycle management: PID file, structured logging, signal handling, and graceful shutdown. The daemon-bootstrap-research (docs/daemon-bootstrap-research.md) covers wiring agents/channels/toolsets. This research focuses on the **missing pieces**: daemon process management, cross-platform signal handling, and CLI commands.
...
| OwlBear daemon-bootstrap-research | `docs/daemon-bootstrap-research.md` | .95 | Daemon loop, model bridge, channel integration (§3.3–3.5) |

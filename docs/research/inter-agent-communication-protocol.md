...
```
kanban\kanban-md.exe create "Implement two-channel protocol in agent-common.instructions.md" --priority needed --tags "scope:copilot,agent,phase-agent-arch" --body "Add ## Inter-agent communication protocol section to agent-common.instructions.md with: routing signal format, task body section format, file-reference threshold rule (1500 tokens), PowerShell escaping guidance. Reference: docs/research/inter-agent-communication-protocol.md"

...
| 8 | OwlBear httpx-timeout-research.md | `docs/httpx-timeout-research.md` | .90 — Timeout conventions per use case |
...
Per `docs/httpx-timeout-research.md`, external API calls with small payloads use 15s (GitHub API) or 30s (URL fetch). Kroki typical latency ~200ms, but complex diagrams can take several seconds. **30s with 5s connect** matches existing `read_url`/`bookmark_pipeline` precedent and the current #617 AC.
...
CORRECTED: POST uses plain text body, NOT deflate+base64 (that is for GET only). See docs/diagram-service-kroki-research.md."
```

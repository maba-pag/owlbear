...
```bash
kanban\kanban-md.exe create "Simplify role system: replace roles.py with inline filtered() in _build_agent" --priority nice-to-have --status backlog --tags "refactor,yagni,scope:core" --body "Replace AgentRole enum, RolePolicy, apply_role_policy, BUILDER_POLICY, VALIDATOR_POLICY with inline toolset.filtered() call. See docs/research/role-system-complexity.md. AC: (1) roles.py deleted, (2) _build_agent uses inline filtered() for non-builder roles, (3) test_roles.py replaced with simpler test in test_agent_registry.py, (4) all existing tests pass, (5) __init__.py exports cleaned up. Depends on: #561 (apply role policy before construction)."

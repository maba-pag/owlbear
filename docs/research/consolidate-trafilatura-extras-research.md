...
| 5 | OwlBear #537 research | `docs/centralize-trafilatura-research.md` | 1.0 |
...
```shell
kanban\kanban-md.exe create "Add pyproject.toml comments for shared trafilatura dep" --priority nice-to-have --status backlog --tags "config,deps,phase-9" --body "Add inline comments to pyproject.toml explaining trafilatura duplication across crawl and search extras (intentional, per docs/consolidate-trafilatura-extras-research.md). Update content_extractor.py import guard to mention both --extra crawl and --extra search. AC: (1) pyproject.toml crawl/search lines have clarifying comments, (2) content_extractor.py ImportError message mentions both extras, (3) uv lock succeeds."
```

...
| OwlBear source-registry-research | docs/source-registry-research.md | 1.0 | `KnowledgeSource` data model, `knowledge_sources` SQLite table, `RefreshOrchestrator` for crawl/ingest dispatch |
| OwlBear content-hashing-research | docs/content-hashing-research.md | .95 | SHA-256 content hash dedup, `check_content_changed()`, `document_status` table |

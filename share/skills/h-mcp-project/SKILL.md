...
Reads `owlbear-project.json` from the current working directory (the project root where VS Code spawns the server). When the file is missing or unparseable, returns a descriptive error string rather than raising — other tools remain functional.
...
All tools follow the `error: ` prefix convention for error strings. No tool raises an exception to the MCP transport — errors are returned as string values.
...
- **`project_info` degrades gracefully.** When `owlbear-project.json` is missing, it returns an error string but the server stays up. Other tools remain functional.

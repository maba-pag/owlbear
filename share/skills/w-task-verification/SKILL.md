...
Score ≤ 2: call `save_memory` with source agent `auditor` for the architect-quality lesson and delegate an architect-calibration follow-up via planner using `Plan and create:` (single-task shortcut at `backlog`).

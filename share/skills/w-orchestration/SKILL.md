...
- **Brief context:** The ideation workflow's parent task body may contain a Brief artifact (Problem, Outcomes, Approach, Scope, Investment Tier). This context is available to pipeline agents via parent task lookup (`show_task(parent_id)`) — the orchestrator does not use Brief context directly.

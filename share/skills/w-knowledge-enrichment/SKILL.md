...
Repeat for all chunks in the batch, pairing each `chunk_id` with the exact `claim_token` returned by `claim_enrichment_batch`, then call `claim_enrichment_batch` for the next batch. If a store call fails for the current claim, the server records diagnostics and moves that chunk to `failed`; correct the payload/extractor issue before calling `retry_enrichment`.
...
- **Lease tokens**: Store every Phase 1 chunk with the `claim_token` returned by `claim_enrichment_batch`. Missing or stale tokens are rejected so parallel workers cannot overwrite one another.
- **Failed chunks**: Failed Phase 1 writes stay in `failed` until `retry_enrichment` resets them. Do not blindly reset; fix the cause first.

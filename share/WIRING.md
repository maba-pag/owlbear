...
| finalizer | GPT-5.6 Luna | `w-change-finalization`, `h-codebase-orientation` | build-reviewer | User-invoked or continuation-issued exact Change proof and finalization; `PreToolUse`: deny writes and terminal mutation |
...
| `design` | `prompt` -> designer in direct design mode | Agent required-reading loads `w-design-session`; rehydrates the same target Design session, including a continuation `resume-design` handoff |
| `orchestrate` | `prompt` -> orchestrator | Agent required-reading loads `w-orchestration`; portfolio batch entry |
| `continue-change` | `prompt` -> orchestrator | Agent required-reading loads `w-orchestration`; Change Continuation Entry acquires one action for the named Change and runs the fixed engine executor |
| `finalize-change` | `prompt` -> finalizer | Agent required-reading loads `w-change-finalization`; engine proof and exact reviewed finalization, including an issued continuation attempt |
...
| finalizer | orchestrator | An issued continuation finalization launch cannot be dispatched; the loop omits the `finalizer` capability and reports the engine-authored `/finalize-change` command instead |

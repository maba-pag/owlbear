...
agents: [shaper-challenger, planner, Explore]
hooks:
  PreToolUse:
    - type: command
...
- **Split only when the builder or verifier would otherwise need unrelated context.** Dependency order beats fake precision.
...
| planner | Decompose an over-broad task into buildable children | `Plan and create: #42 — split cockpit API and frontend changes` |
...
| Verdict | Format |
|---------|--------|
| Approve | `APPROVED #{id} -> build \| {one-line summary}` |

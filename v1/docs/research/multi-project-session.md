...
| OwlBear knowledge-scoping-research | docs/research/knowledge-scoping.md | .95 | Column-based scope filtering with `scope: str` on all knowledge models. Format: `"global"`, `"project:{name}"`, `"agent:{name}"`. Already designed and partially implemented (schema v4 has scope columns). |
...
Already designed (docs/research/knowledge-scoping.md §4). Knowledge models have `scope: str = "global"`. For multi-project:
...
|--------|-----|------|------------|------|
| Devika ProjectManager | github.com/stitionai/devika/blob/main/src/project.py | Per-project conversation storage, project list/create/delete CRUD pattern, project-to-directory mapping | docs/research/multi-project-session.md (§3.1–3.3 design), future src/owlbear/projects/ | 2026-03-01 |
| Devika AgentState | github.com/stitionai/devika/blob/main/src/state.py | Project-keyed state management pattern, per-project token tracking | docs/research/multi-project-session.md (§3.8 switching design) | 2026-03-01 |
| Nanobot SessionManager | github.com/HKUDS/nanobot/blob/main/nanobot/session/manager.py | Workspace-scoped JSONL sessions, session key format, cache + persist pattern | docs/research/multi-project-session.md (§3.4 session binding) | 2026-03-01 |
| Nanobot ContextBuilder | github.com/HKUDS/nanobot/blob/main/nanobot/agent/context.py | Workspace-as-project-root, bootstrap files from workspace, memory from workspace dir | docs/research/multi-project-session.md (§3.5 context isolation) | 2026-03-01 |
| Mem0 memory scoping | github.com/mem0ai/mem0 | Multi-level memory with user_id/agent_id filter parameters at query time | docs/research/multi-project-session.md (§3.6 knowledge scoping) | 2026-03-01 |

...
class TestRoleValues:
    """Each definition's role is a valid string ('builder' or 'validator')."""

    @pytest.mark.parametrize("agent_name", list(EXPECTED_AGENTS))
    def test_role_is_valid_string(self, agent_name: str) -> None:
        defn = parse_agent_definition(AGENTS_DIR / f"{agent_name}.md")
        assert defn.role in ("builder", "validator")

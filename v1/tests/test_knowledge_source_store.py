...
class TestFromAC_QueryBehaviorPredicates:
    ...
    def test_get_discriminates_by_id_not_by_position(self, store: KnowledgeSourceStore) -> None:
        ...
        assert got is not None
        assert got.id == src2.id, f"get(src2.id) should return src2, but returned {got.id!r}"
    ...
    def test_get_by_name_sql_where_clause_filters_by_name_and_scope(
        ...
        stmts = _select_stmts_on_ks(executed)
        assert stmts, "get_by_name() emitted no SELECT on knowledge_sources"
        assert any(re.search(r"WHERE name =", s) and re.search(r"AND scope =", s) for s in stmts), (

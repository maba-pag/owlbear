        ...
        for pattern in self._blocked_commands:
            if re.search(pattern, cmd):
                self._emit_audit_event(
                    ...
                    detail=f"Blocked command: {cmd}",
                    metadata={"pattern": pattern, "value": cmd, "target": "command"},
                )
        ...
        for pattern in self._blocked_file_patterns:
            if re.search(pattern, path):
                self._emit_audit_event(
                    ...
                    detail=f"Blocked file path: {path}",
                    metadata={"pattern": pattern, "value": path, "target": "path"},
                )
    ...
    def _emit_audit_event(
        ...
        *,
        ...
        try:
            ...
            logger.debug("Security audit sink failed in CommandSafetyGuard", exc_info=True)

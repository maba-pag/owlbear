...
def _dispatch_shipped_planner(revision, started, *, receipt_id: str, next_job_id: int) -> dict[str, object]:
    ...
    assert "execute/runInTerminal" in planner["tools"]
    assert planner["hooks"]["PreToolUse"][0]["command"].endswith("deny-writes.py --terminal-read-only")
    ...
    assert all(
        ...
        for requirement in (
            ...
            "the review has disproved verification-only eligibility",
            "Redraft the candidate in build mode with a\nbounded packet",
            "remove escaped paths rather than borrowing their module",
            "Do not return `PlanBlocked` merely because the current tracked",

...
class TestFromAC_ListSessions:
    ...
    def test_claim_to_end_work_is_one_session(
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=1, detail="success: todo -> in-progress"
                ),
            ],
        )
    ...
    def test_end_work_block_maps_to_completed_fail(
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=3, detail="blocked: needs DB migration"
                ),
            ],
        )
        sessions = engine.list_sessions(filter="all")
    ...
    def test_end_work_reject_state_is_completed_rejected(
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=4, detail="reject: in-progress -> todo"
                ),
            ],
        )
        sessions = engine.list_sessions(filter="all")
    ...
    def test_release_event_state_is_released(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_open_recent_claim_state_is_running(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_old_claim_no_activity_state_is_stuck(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_default_filter_returns_running_and_stuck_only(
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=12, detail="success: todo -> in-progress"
                ),
            ],
        )
    ...
    def test_filter_all_returns_every_session(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=21, detail="success: todo -> in-progress"
                ),
                _entry(
                    action="claim",
                    task_id=22,
    ...
    def test_filter_failed_or_rejected_includes_fail_and_reject(
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=31, detail="reject: in-progress -> todo"
                ),
            ],
        )
    ...
    def test_filter_failed_or_rejected_excludes_pass_and_released(
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=32, detail="success: todo -> in-progress"
                ),
                _entry(
                    action="claim",
                    task_id=33,
    ...
    def test_filter_released_returns_released_sessions_only(
        ...
        _write_log(
            ...
            [
                ...
                _entry(
                    action="end_work", task_id=41, detail="success: todo -> in-progress"
                ),
            ],
        )
        ...
        assert states <= {"released"}, (
            "released filter must return only released-state sessions"
        )
    ...
    def test_empty_activity_log_returns_empty_list(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_missing_activity_log_returns_empty_list(
    ...
    def test_malformed_json_line_skipped_gracefully(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
        ...
        valid_close = _entry(
            action="end_work", task_id=50, detail="success: todo -> in-progress"
        )
        with log_path.open("w", encoding="utf-8") as f:
    ...
    def test_incomplete_entry_missing_fields_skipped_gracefully(
        ...
        sessions = engine.list_sessions(filter="all")
        returned_ids = {s.task_id for s in sessions}
        assert 61 in returned_ids, (
            "valid entry following incomplete entry must be processed"
        )
    ...
    def test_reclaim_after_release_produces_two_distinct_sessions(
        ...
        assert len(task_sessions) == 2, (
            f"expected 2 distinct sessions for task 70, got {len(task_sessions)}"
        )
...
class TestBuilderDiscovered:
    ...
    def test_invalid_timestamp_value_skipped_gracefully(
        ...
        sessions = engine.list_sessions(filter="all")
        returned_ids = {s.task_id for s in sessions}
        assert 90 in returned_ids, (
            "valid entry before bad-timestamp entry must be processed"
        )
        assert 91 not in returned_ids, (
            "entry with unparseable timestamp must be skipped"
        )
        assert 92 in returned_ids, (
            "valid entry after bad-timestamp entry must be processed"
        )
    ...
    def test_young_superseded_claim_is_running_not_stuck(
        ...
        assert len(task_sessions) == 2, (
            "two consecutive claims must produce two distinct sessions"
        )
        states = [s.state for s in task_sessions]
        ...
        assert "running" in states, (
            "young superseded claim must be 'running' (age < timeout)"
        )
...
class TestFromAC_WorkSessionFields:
    ...
    def test_worksession_exported_from_package(self) -> None:
        ...
        assert hasattr(mod, "WorkSession"), (
            "WorkSession must be exported from owlbear_kanban package"
        )
    ...
    def test_session_has_agent_field(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_session_has_started_at_field(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_session_has_duration_field(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_session_has_outcome_field(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_agent_equals_claim_event_detail(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_agent_is_detail_not_actor(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_started_at_equals_claim_timestamp(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_started_at_is_iso8601_string(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_duration_is_close_minus_claim_seconds(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_duration_is_none_for_running_session(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_duration_is_none_for_stuck_session(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_duration_is_none_for_superseded_claim(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_duration_with_tz_naive_claim_timestamp(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_duration_is_set_for_released_session(
        self, engine: KanbanEngine, log_path: Path
    ) -> None:
    ...
    def test_superseded_claim_started_at_from_original_claim_timestamp(
        ...
        _write_log(
            ...
            [
                _entry(
                    action="claim", task_id=251, detail="original-agent", ts=original_ts
                ),

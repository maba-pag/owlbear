...
def _make_dispatch(**kwargs: object) -> DispatchEntry:
    base = {"id": 1, "status": "todo", "priority": "needed", "title": "T", "tags": [], "agent": "builder"}
...
class TestFromAC_ErrorCodeCatalogue:
    ...
    def test_err_move_to_forbidden_on_release(self) -> None:
        e = ValidationError(code="ERR_MOVE_TO_FORBIDDEN_ON_RELEASE", user_message="move_to forbidden on release")
    ...
    def test_err_archived_not_claimable(self) -> None:
        e = ValidationError(code="ERR_ARCHIVED_NOT_CLAIMABLE", user_message="archived task not claimable")

"""Tests for task #944: Implement _id_to_filename cache for show_task() and _find_task_path().

AC coverage:
  1. __init__ adds _id_to_filename: dict[int, str] = {}
  2. list_tasks() rebuilds _id_to_filename from _task_cache after scandir loop
  3. refresh_config() clears _id_to_filename
  4. show_task() checks _id_to_filename → _task_cache → stat validate → fallback to glob
  5. show_task() evicts stale entries on stat() FileNotFoundError
  6. _find_task_path() checks _id_to_filename → return path → fallback to glob
  7. No archive dispatch in _find_task_path() (all callers use tasks_dir)
  8. All existing tests pass + new tests from RED phase pass

Focus: edge cases and interaction scenarios not covered by test_idtofilename_cache_943.py.
All tests must FAIL in RED phase — _id_to_filename not yet implemented.
"""
...
# ---------------------------------------------------------------------------
# Board helpers — mirrors conventions from test_idtofilename_cache_943.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TestFromAC_IdToFilenameCache_944
# ---------------------------------------------------------------------------

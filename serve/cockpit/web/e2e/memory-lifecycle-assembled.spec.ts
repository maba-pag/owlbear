...
const EXCEPTIONAL_ENTRIES = [
  {
    ...
    state: "contested",
    title: "Contested memory",
  },
  {
    id: "33333333-3333-4333-8333-333333333333",
    state: "disputed",
    title: "Disputed memory",
  },
  {
...
test.describe("assembled Memory lifecycle", () => {
  ...
  for (const viewport of [
    ...
    test(`${viewport.name} proves lifecycle visibility and human controls`, async ({
      page,
    }) => {
      ...
      await expect(page.getByTestId("memory-entry-title")).toHaveText(
        ORDERED_TITLES,
      );
      await expect(page.getByTestId("memory-entry-score")).toHaveText([
        "0.95",
        "0.88",
        "0.84",
        "0.82",
      ...
      for (const score of await page.getByTestId("memory-entry-score").all()) {
        expect(
          await score.evaluate(
            (element) =>
              (element as HTMLElement & { variant?: string }).variant,
          ),
        ).toBe("secondary");
      }

      const approved = await openEntry(page, "Approved memory");
      await expect(approved.getByTestId("memory-content-panel")).toContainText(
        "Lifecycle fixture content for Approved memory.",
      );
      ...
      await expect(contested.getByTestId("memory-entry-state")).toHaveText(
        "contested",
      );
      const contestedTask = contested
        .getByText("Contested task", { exact: true })
        .locator("..");
      ...
      await contested
        .getByTestId("memory-entry-title")
        .scrollIntoViewIfNeeded();
      await page.screenshot({
        path: `test-results/memory-lifecycle-${viewport.name}.png`,
        fullPage: true,
      });
      if (viewport.name === "mobile") {
        await contested
          .getByTestId("memory-detail-actions")
          .scrollIntoViewIfNeeded();
        await page.screenshot({
          path: "test-results/memory-lifecycle-mobile-actions.png",
          fullPage: true,
        });
      } else {
        await contested.screenshot({
          path: "test-results/memory-lifecycle-desktop-detail.png",
        });
      }
  ...
  test("real MCP and Cockpit boundaries reject or complete exceptional-state operations", async ({
    page,
  }) => {
    const mcpProofPath = resolve(
      process.cwd(),
    ...
    expect(mcpProof.responses.map((response) => response.state)).toEqual([
      "contested",
      "disputed",
      "stale",
    ]);
    ...
    for (const exceptional of EXCEPTIONAL_ENTRIES) {
      ...
      await entry
        .locator('p-input-text[name="edit-title"]')
      ...
      const editResponsePromise = page.waitForResponse(
        (response) =>
          ...
          response.url().endsWith(`/api/memories/${exceptional.id}/edit`),
      );
      await entry.getByTestId("memory-edit-save-btn").click();
      const editResponse = await editResponsePromise;
      expect(editResponse.status()).toBe(200);
      const editPayload = (await editResponse.json()) as {
        entry: { id: string; state: string; title: string };
      };
      expect(editPayload.entry).toMatchObject({
        id: exceptional.id,
        state: exceptional.state,
        title: editedTitle,
      });

      entry = page.getByTestId("memory-entry").filter({ hasText: editedTitle });
      await expect(entry.getByTestId("memory-entry-title")).toHaveText(
        editedTitle,
      );
      const resolveResponsePromise = page.waitForResponse(
        (response) =>
          response.request().method() === "POST" &&
          response.url().endsWith(`/api/memories/${exceptional.id}/resolve`),
      );
      ...
      expect(resolvePayload.entry).toMatchObject({
        id: exceptional.id,
        state: "approved",
        title: editedTitle,
      });
      await expect(entry.getByTestId("memory-entry-state")).toHaveText(
        "approved",
      );

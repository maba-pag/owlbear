...
const SCOPE_ENTRIES = [
  ...
  {
    id: "memory-all",
    title: "All Agents Memory",
    content: "All agents content",
    categories: ["pitfall"],
    confidence: 0.82,
    score: 0.82,
    state: "curated",
  ...
  {
    id: "memory-all-builder",
    title: "All Builder Memory",
    content: "Wildcard builder content",
    categories: ["process"],
    confidence: 0.73,
    score: 0.73,
    state: "approved",
  ...
  {
    id: "memory-pair",
    title: "Named Pair Memory",
    content: "Builder reviewer content",
    categories: ["preference"],
    confidence: 0.55,
    score: 0.55,
    state: "curated",
  ...
  {
    id: "memory-mode-all",
    title: "Mode All Memory",
    content: "Collision-safe content",
    categories: ["process"],
    confidence: 0.46,
    score: 0.46,
    state: "approved",
    scope_agents: ["mode:all"],
    source_agent: "builder",
    ...
    route.fulfill({
      json: {
        ...
        modules: Object.fromEntries(
          ["tasks", "requests", "memory", "ideas"].map((name) => [
            name,
            {
              status: "healthy",
              findings: [],
...
async function selectAgent(page: Page, value: string): Promise<void> {
  await page
    .locator('p-select[name="agent-filter"]')
    .evaluate((element, selectedValue) => {
...
test.describe("Memory state filter", () => {
  ...
  test("state multi-select change event controls visible rows and shown count", async ({
    page,
  }) => {
    await stubApis(page);
    ...
    await expect(page.getByTestId("workspace-header-metric")).toHaveText(
      /3\s*of\s*4\s*shown/,
    );
...
test.describe("Memory scope filter", () => {
  ...
  test("agent modes preserve scope semantics, composition, labels, and shown count", async ({
    page,
  }) => {
    ...
    const titles = page.getByTestId("memory-entry-title");
    const agentOptions = page.locator(
      'p-select[name="agent-filter"] p-select-option',
    );
    await expect(agentOptions).toHaveText([
      "Any agent",
      "All agents",
      "Unscoped",
      "builder",
    ...
    await page
      .locator('p-multi-select[name="category-filter"]')
      .evaluate((element) => {
        element.dispatchEvent(
          new CustomEvent("change", {
    ...
    await page
      .locator('p-input-search[name="memory-search"]')
      .evaluate((element) => {
        element.dispatchEvent(
    ...
    await expect(page.getByTestId("memory-entry-agents").nth(0)).toHaveText(
      "All agents",
    );
    await expect(page.getByTestId("memory-entry-agents").nth(2)).toHaveText(
      "mode:all",
    );
    ...
    await expect(
      page
        .getByTestId("memory-accordion-detail")
        .getByText("Unscoped", { exact: true }),
    ).toBeVisible();
  });
});

...
const SCOPE_ENTRIES = [
  {
    id: 'memory-unscoped', title: 'Unscoped Memory', content: 'Unscoped content', categories: ['process'], confidence: 0.91,
  ...
  {
    id: 'memory-all', title: 'All Agents Memory', content: 'All agents content', categories: ['pitfall'], confidence: 0.82,
  ...
  {
    id: 'memory-all-builder', title: 'All Builder Memory', content: 'Wildcard builder content', categories: ['process'], confidence: 0.73,
  ...
  {
    id: 'memory-builder', title: 'Builder Memory', content: 'Builder content', categories: ['process'], confidence: 0.64,
  ...
  {
    id: 'memory-pair', title: 'Named Pair Memory', content: 'Builder reviewer content', categories: ['preference'], confidence: 0.55,
  ...
  {
    id: 'memory-mode-all', title: 'Mode All Memory', content: 'Collision-safe content', categories: ['process'], confidence: 0.46,
...
test.describe('Memory scope filter', () => {
  ...
  test('agent modes preserve scope semantics, composition, labels, and shown count', async ({ page }) => {
    ...
    await expect(agentOptions.evaluateAll((options) => options.map((option) => option.getAttribute('value')))).resolves.toEqual([
      'mode:any', 'mode:all', 'mode:unscoped', 'agent:builder', 'agent:mode%3Aall', 'agent:reviewer',
    ])
    ...
    await expect(page.getByTestId('memory-accordion-detail').getByText('Unscoped')).toBeVisible()
  })
})

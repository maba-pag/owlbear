/**
 * RED-phase Playwright E2E tests for #1593: Board horizontal scroll.
 *
 * BUILDER INSTRUCTION (#1596): Do not edit this file — implement the fix in
 * KanbanBoard.tsx (change repeat(auto-fit,...) to repeat(N,...)). These tests
 * must remain failing until the implementation lands.
 *
 * Expected to FAIL against the current implementation:
 *   Board grid uses `repeat(auto-fit, minmax(200px, 1fr))` → at default viewport
 *   (1280×720) the workspace is ~864px wide. 7 columns at 200px min = 1400px, but
 *   auto-fit instead wraps excess columns to new rows → scrollWidth ≈ clientWidth
 *   (no horizontal overflow). All columns on different rows → different offsetTop.
 *
 * Post-fix expected state (#1596):
 *   Grid changed to fixed repeat(7, minmax(200px, 1fr)) → single row, scrollWidth
 *   = 1400px > clientWidth ≈ 864px → horizontal scroll. All offsetTop identical.
 *
 * API mocking: all routes stubbed via page.route() — no real backend required.
 * Viewport: default Playwright Desktop Chrome (1280×720).
 */
...
test.describe('TestFromAC_BoardHorizontalScroll', () => {

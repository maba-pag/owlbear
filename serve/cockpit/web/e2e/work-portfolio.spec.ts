...
async function inspect(
  ...
  title: string,
  detailTitle = title,
): Promise<{ detail: Locator; trigger: Locator }> {
  ...
  await expect(
    detail.getByRole("heading", { name: detailTitle, exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("dialog", { name: "Work Item detail" }),
  ).toBeVisible();
...
async function selectValue(locator: Locator, value: string): Promise<void> {
  await locator.evaluate((element, selected) => {
    ...
    element.dispatchEvent(
      new CustomEvent("change", { detail: { value: selected }, bubbles: true }),
    );
  }, value);
}
...
async function inputValue(locator: Locator, value: string): Promise<void> {
  await locator.evaluate((element, input) => {
    ...
    element.dispatchEvent(
      new CustomEvent("input", { detail: { value: input }, bubbles: true }),
    );
  }, value);
}
...
test.describe("assembled Delivery portfolio", () => {
  ...
  test("wide workspace explains operating state and provides routed detail", async ({
    page,
  }, testInfo) => {
    ...
    await expect(
      guidance
        ...
        .getByText("/design design-operations-roadmap", { exact: true }),
    ).toBeVisible();
    ...
    const publicationRow = page.getByLabel(
      "Change publication for Publication release",
    );
    ...
    const normalOutcome = (await visibleRows(page))
      .filter({ hasText: "Publish operator guide" })
      .locator("td")
      .first();
    ...
    expect(publicationHitTarget.href).toBe(
    ...
    await expect
      .poll(() =>
        publicationTrigger.evaluate((element) =>
    ...
    await expect(
      designSection.getByRole("heading", { name: "Design work" }),
    ...
    await expect(designDetail).toContainText(
    ...
    await selectValue(
      page.locator('p-select[name="work-needs-filter"]'),
      "you",
    );
    ...
    await expect(
      returnedDetail.getByRole("heading", { name: "Plan release notes" }),
    ).toBeVisible();
    await expect(returnedDetail).toContainText("Returned to Design");
    await expect(returnedDetail).toContainText(
      "Evidence: request:release-notes-authority",
    );
    await expect(returnedDetail).toContainText(
      "Source boundary: design:release-notes-v2",
    );
    ...
    await expect(requestAction).toHaveJSProperty(
      "href",
      "/delivery/work-e2e/outcome%3AOUT-001",
    );
    ...
    await expect(
      inspected.detail.getByRole("heading", { name: "Choose release mode" }),
    ).toBeVisible();
    await expect(page.getByTestId("work-portfolio-table")).toBeVisible();
    await expect(inspected.detail).toContainText(
      "Resolve the bounded release decision.",
    );
    await expect(inspected.detail).toContainText(
      "Observe Choose release mode.",
    );
    await expectNoHorizontalOverflow(page);
    await selectValue(
      inspected.detail.locator(
        'p-select[name="request-request-release-mode-option"]',
      ),
      "safe",
    );
    const answerResponse = page.waitForResponse(
      (response) =>
        ...
        response.url().includes("/requests/request-release-mode/answer"),
    );
    ...
    await expect(inspected.detail).toContainText(
      "Finalize the reviewed Change",
    );
    await expect(
      inspected.detail.getByTestId("publication-readiness-status"),
    ).toHaveText("Ready");
    await expect(
      inspected.detail.locator(
        'section[aria-labelledby="work-publication-heading"] [data-section-tone="neutral"]',
      ),
    ).toBeVisible();
    await expect(
      inspected.detail.locator('[data-section-tone="warning"]'),
    ).toHaveCount(0);
    ...
    await inspected.detail
      .getByText("Administrative actions", { exact: true })
      .click();
    await expect
      .poll(() =>
        page
    ...
    await selectValue(
      inspected.detail.locator('p-select[name="backward-stage"]'),
      "planning",
    );
    await inputValue(
      inspected.detail.locator('p-input-text[name="backward-reason"]'),
      "Authority changed",
    );
    const previewResponse = page.waitForResponse(
      (response) =>
        response.request().method() === "POST" &&
        response.url().endsWith("/move-backward/preview"),
    );
    await inspected.detail
      .getByText("Review backward move", { exact: true })
      .click();
    expect((await previewResponse).status()).toBe(200);
    const modal = page
      .locator("p-modal")
      .filter({ hasText: "The following Outcomes will be reset:" });
    await expect(modal).toContainText("OUT-003");
    const moveResponse = page.waitForResponse(
      (response) =>
        response.request().method() === "POST" &&
        response.url().endsWith("/outcomes/OUT-003/move-backward"),
    );
    await modal.getByText("Confirm backward move", { exact: true }).click();
    expect((await moveResponse).status()).toBe(200);
    await expect(inspected.detail).toContainText(
      "Moved backward. Reset: OUT-003.",
    );
    await returnToPortfolio(page, inspected.trigger);
  ...
  test("compact workspace uses a fullscreen flyout and restores row focus", async ({
    page,
  }, testInfo) => {
    ...
    const guidance = page.getByLabel("Delivery guidance");
    await expect(
      guidance.getByTestId("portfolio-next-session").locator("li"),
    ).toHaveCount(3);
    await expect(page.getByTestId("work-portfolio-table")).toBeVisible();
    await expectNoHorizontalOverflow(page);
    ...
    const compactCommandLayout = await compactCommand.evaluate((button) => {
      const code = button.querySelector("code");
      const icon = button.querySelector("p-icon") as
    ...
    await expect(
      page.getByRole("button", { name: "Dismiss flyout" }),
    ).toBeVisible();
    await expect(inspected.detail).toContainText("Build OUT-002");
  ...
  test("mobile rows preserve field semantics without overflow", async ({
    page,
  }, testInfo) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto("/delivery");
    ...
    await expect(publicationRow.locator("dd")).toHaveCount(2);
    await expect(publicationRow).toHaveAttribute(
      "aria-label",
      "Change publication for Publication release",
    );
    await expectNoHorizontalOverflow(page);
  ...
  test("filter popover preserves portfolio position and supports real selection", async ({
    page,
  }) => {
    for (const width of [1177, 1440]) {
      ...
      await page
        .getByRole("option", { name: "Needs you", exact: true })
        .click();
      ...
      await expect(page.getByTestId("work-filters-panel")).toBeVisible();
      await page
        .getByRole("heading", { name: "Work portfolio E2E", exact: true })
        .click();
      await expect(page.getByTestId("work-filters-panel")).not.toBeVisible();
      await expect(toggle).toBeFocused();
    }
  ...
  test("filter popover fits the compact viewport without horizontal overflow", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto("/delivery");
  ...
  test("desktop workspaces keep columns and overview context beside detail", async ({
    page,
  }) => {
    for (const width of [1024, 1280]) {
      ...
      await expect(table).toBeVisible();
      await expect(
        table.getByRole("columnheader", { name: "Work" }),
      ).toBeVisible();
      const inspected = await inspect(page, "Build operator controls");
      await expect(inspected.detail).toContainText("Build OUT-002");
      const flyoutBox = await flyoutPanelBox(page);
  ...
  test("wide detail flyout preserves overview context without horizontal scrolling", async ({
    page,
  }) => {
  ...
  test("routed detail survives reload and completed history remains reachable", async ({
    page,
  }, testInfo) => {
    ...
    const detail = page.getByTestId("work-item-detail");
    await expect(
      detail.getByRole("heading", { name: "Publish operator guide" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByTestId("desktop-product-navigation").getByLabel("Delivery"),
    ).toHaveAttribute("aria-current", "page");

    await page.getByRole("button", { name: "Dismiss flyout" }).click();
    await expect(
      page.getByRole("button", { name: "Current delivery", exact: true }),
    ).toBeFocused();
    const historyResponse = page.waitForResponse(
      (response) =>
        response.request().method() === "GET" &&
        new URL(response.url()).pathname === "/api/work-items/completed",
    );
    ...
    await expect(
      page.getByText("Alpha delivery", { exact: true }),
    ).toBeVisible();
    const receiptRecord = page
      .getByTestId("completed-change-record")
      .filter({ hasText: "Beta search" });
  ...
  test("closing detail does not leave a history entry that reopens it", async ({
    page,
  }) => {
    await page.goto("/delivery");
  ...
  test("browser Back from detail restores the originating trigger focus", async ({
    page,
  }) => {
    await page.goto("/delivery");
  ...
  test("refreshes current delivery immediately after returning from completed history", async ({
    page,
  }) => {
    ...
    await page.route("**/api/work-items", async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          groups: firstGroup
            ? [
                { ...firstGroup, title: "Refreshed portfolio" },
                ...remainingGroups,
              ]
            : payload.groups,
        }),
    ...
    try {
      await page.goto("/delivery");
      await expect(
        page.getByText("Work portfolio E2E", { exact: true }),
      ).toBeVisible();
      await page.getByRole("button", { name: "Change history" }).click();
      await expect(
        page.getByTestId("completed-history-workspace"),
      ).toBeVisible();

      refreshPortfolio = true;
      await page.getByRole("button", { name: "Current delivery" }).click();
      await expect(
        page.getByText("Refreshed portfolio", { exact: true }),
  ...
  test("completed history detail dismisses on Escape and restores record focus", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1440, height: 760 });
    await page.goto("/delivery");
  ...
  test("browser Back from completed history detail restores record focus", async ({
    page,
  }) => {
    await page.goto("/delivery");
    await page.getByRole("button", { name: "Change history" }).click();
  ...
  test("completed history detail survives reload and restores workspace focus", async ({
    page,
  }) => {
    await page.goto("/delivery");
    await page.getByRole("button", { name: "Change history" }).click();
  ...
  test("completed history detail closes with its Close button and restores record focus", async ({
    page,
  }) => {
    await page.goto("/delivery");
    await page.getByRole("button", { name: "Change history" }).click();
  ...
  test("completed history detail closes from the backdrop and restores record focus", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1440, height: 760 });
    await page.goto("/delivery");
  ...
  test("completed history restores focus when a pending search replaces the open detail trigger", async ({
    page,
  }) => {
    ...
    try {
      ...
      const searchInput = page
        .locator('p-input-search[name="completed-history-search"]')
        .locator("input");
      await searchInput.fill("Beta");
      await expect(
        page.getByTestId("completed-history-stale-status"),
      ).toBeVisible();

      const receiptRecord = page
        .getByTestId("completed-change-record")
        .filter({ hasText: "Alpha delivery" });
      const trigger = receiptRecord.getByRole("button", { name: "Inspect" });
      await trigger.click();
      await expect(page.getByTestId("completed-change-detail")).toContainText(
        "Completion receipt",
      );
      ...
      await expect(
        page.getByTestId("completed-change-detail"),
      ).not.toBeVisible();
      await expect(
        page.getByTestId("completed-history-workspace"),
      ).toBeFocused();
  ...
  test("completed history focuses its workspace when detail closes during a pending search", async ({
    page,
  }) => {
    let releaseSearch: (() => void) | undefined;
    const searchBlocked = new Promise<void>((resolve) => {
  ...
  test("completed history search remains editable while results are pending", async ({
    page,
  }) => {
    let releaseSearch: (() => void) | undefined;
    const searchBlocked = new Promise<void>((resolve) => {
    ...
    try {
      ...
      const search = page.locator(
        'p-input-search[name="completed-history-search"]',
      );
      ...
      await expect(
        page.getByTestId("completed-history-results"),
      ...
      await expect(
        page.getByText("Beta search", { exact: true }),
      ).not.toBeVisible();
    } finally {
      releaseSearch?.();
      await page.unroute("**/api/work-items/completed/search**");
  ...
  test("automatically removed Work Item routes to history and restores view focus", async ({
    page,
  }) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          groups: payload.groups.filter(
            (group) => group.change_id !== "work-e2e",
          ),
        }),
      });
    });
    ...
    try {
      ...
      await page.waitForResponse(
        (response) =>
          response.request().method() === "GET" &&
          new URL(response.url()).pathname === "/api/work-items",
      );
      await expect(
        page.getByTestId("completed-history-workspace"),
      ).toBeVisible();
  ...
  test("automatically closes removed Design detail and restores current view focus", async ({
    page,
  }) => {
    ...
    await page.route("**/api/work-items", async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          operating: {
            ...payload.operating,
            statuses: payload.operating.statuses.filter(
              (status) => status.change_id !== "design-operations-roadmap",
            ),
          },
        }),
      });
    ...
    try {
      ...
      await expect(
        page.getByRole("button", { name: "Current delivery" }),
      ).toBeFocused();
    } finally {
      await page.unroute("**/api/work-items");
    }
  ...
  test("unknown paths render the global Not Found view", async ({
    page,
  }, testInfo) => {

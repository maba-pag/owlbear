...
function applyTokenFallbacks(): void {
  ...
  if (!computed.getPropertyValue("--p-font-porsche-next").trim()) {
    rootStyle.setProperty(
      "--p-font-porsche-next",
      '"Porsche Next", "Helvetica Neue", Arial, sans-serif',
    );
  }
...
function installPdsCdnTrap(): void {
  const docWithPds = document as {
    porscheDesignSystem?: Record<string, unknown>;
  };
...
function waitForRequiredPdsElements(): Promise<void> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timeoutId = setTimeout(() => {
      reject(
        new Error(
          `Porsche Design System components did not load within ${PDS_BOOTSTRAP_TIMEOUT_MS} ms`,
        ),
      );
  ...
  return Promise.race([
    Promise.all(
      REQUIRED_PDS_ELEMENTS.map((tagName) =>
...
function renderBootstrapFailure(): void {
  ...
  message.textContent =
    "The design system components did not finish loading. Reload the page to try again.";

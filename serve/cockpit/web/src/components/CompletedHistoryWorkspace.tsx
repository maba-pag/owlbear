...
function parseSelection(pathname: string): CompletedHistorySelection | null {
  ...
  try {
    return {
      ...
      completionId: decodeURIComponent(parts[3]),
    };
  } catch {
    return null;
  }
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-static-sm">
        <div>
          <h2 className="m-0 text-2xs font-semibold uppercase tracking-[0.08em] text-contrast-high">
            {abandoned ? "Abandoned change" : "Completed change"}
          </h2>
          <p className="mt-static-xs text-sm text-contrast-medium">
            {abandoned
              ? "Why this Change ended before completion and what terminal evidence remains."
      ...
      <PHeading tag="h3" size="small" className="mt-static-md">
        {record.title}
      </PHeading>
      <section className="mt-static-lg grid gap-static-xs">
        ...
        {outcomePromises.length > 0 ? (
          ...
          <p className="text-sm leading-relaxed text-contrast-medium">
            Purpose details were not retained in this completion record.
          </p>
        )}
      </section>
      <section className="mt-static-lg grid gap-static-xs">
        ...
        <section className="mt-static-lg grid gap-static-xs">
          ...
          <dl className="grid gap-static-sm text-sm">
            <div>
              ...
              <dd className="text-contrast-medium">{record.prior_stage}</dd>
            </div>
            <div>
      ...
      <details className="mt-static-lg border-t border-contrast-low pt-static-sm">
        <summary className="cursor-pointer text-2xs font-semibold uppercase tracking-[0.08em] text-contrast-high">
          Evidence
        </summary>
        ...
        <>
          ...
          {confirmOpen ? (
            <PModal
              ...
              aria={{
                role: "alertdialog",
                "aria-label": "Confirm abandoned worktree cleanup",
              }}
            >
              <div className="grid w-[min(32rem,calc(100vw-2rem))] gap-static-md text-primary">
                ...
                <p className="text-sm">
                  {record.target_sync_conflict
                    ? "Delivery will discard the preserved target merge before removing the abandoned worktree."
...
export default function CompletedHistoryWorkspace() {
  ...
  useEffect(() => {
    ...
    const loadMoreButton = historyWorkspace.current?.querySelector<HTMLElement>(
    ...
    >
      <WorkspaceViewHeader
        ...
        meta={
          <WorkspaceViewCount
            separator
            value={history.page.total_count}
            unit={history.page.total_count === 1 ? "change" : "changes"}
          />
        }
        tools={
          <PInputSearch
      ...
      {history.isLoading && history.page.records.length === 0 ? (
        <p className="mt-static-lg" role="status">

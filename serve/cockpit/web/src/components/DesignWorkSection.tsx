...
export default function DesignWorkSection({ changeIds, selectedChangeId, onSelect }: DesignWorkSectionProps) {
  ...
  return (
    <section className="min-w-0" aria-labelledby="design-work-heading" data-testid="design-work-section">
      <h2 id="design-work-heading" className="mb-static-sm px-static-sm text-md font-semibold text-primary">Design work</h2>
        ...
        {changeIds.map((changeId) => {
          ...
          return (
            <article
              key={changeId}
              className={[
                'relative grid min-w-0 gap-static-sm rounded-sm border border-contrast-low px-static-sm py-static-sm text-sm lg:grid-cols-[minmax(0,40fr)_minmax(0,25fr)_minmax(0,35fr)] lg:gap-0',

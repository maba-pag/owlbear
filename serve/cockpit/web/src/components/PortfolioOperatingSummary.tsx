      ...
      >
        ...
        >
          <AttentionMetric
            ...
            icon="warning"
            label="Needs you"
            selected={needsFilter === "you"}
            onSelect={onNeedsFilter}
          />
          <AttentionMetric
            count={totals.needs.dependency}
            filter="dependency"
            icon="clock"
            label="Blocked"
    ...
    >
      {operating.guidance.length > 0 ? (
        <>
          ...
          <ul
            className="m-0 flex min-w-0 flex-wrap items-center gap-x-static-xl gap-y-static-xs p-0"
            data-testid="portfolio-next-session"
          >
            {operating.guidance.map((guidance) => (
              <li className="min-w-0" key={guidance.kind}>
      ...
      {commands.length > 0 ? (
        <>
          ...
          <ul
            className="m-0 flex min-w-0 flex-wrap items-center gap-x-static-lg gap-y-static-xs p-0"
            data-testid="portfolio-commands"
          >
            {commands.map((command) => (
              <li className="min-w-0" key={command}>

...
function portfolio(
  changeIds: string[],
  lifecycle: "awaiting-merge" | "acceptance" = "awaiting-merge",
    ...
    items: [
      {
        ...
        action: {
          kind: "observe-acceptance" as const,
...
function Harness({
  ...
  portfolioData: WorkItemPortfolioResponse;
  onChanged: () => void;
...
it("backs off locally after provider-unavailable outcomes", async () => {
  ...
  render(
    <Harness
      portfolioData={portfolio(["change-a"])}
      onChanged={onChanged}
      onStatus={(status) => {
        latestStatus = status;
      }}

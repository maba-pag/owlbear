...
function MemoryTab() {
  ...
  useEffect(() => {
    ...
    const onUpdate = (event: Event) => {
      setFilter((previous) => ({
        ...previous,
        categories: readStringArrayValue(event),
      }));
    };
  ...
  useEffect(() => {
    ...
    const onUpdate = (event: Event) => {
      setFilter((previous) => ({
        ...previous,
        agent: parseAgentFilterValue(readStringValue(event)),
      }));
    };
    ...
    >
      ...
      {purgeFlow.receipt ? (
        ...
        >
          ...
          <PButton
            ...
            compact
            variant="secondary"
            data-testid="memory-purge-receipt-dismiss"
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-static-lg px-static-lg py-static-lg">
        ...
        {!hasFetched && isFetching ? (
          <div data-testid="memory-loading" role="status">
            Collecting memory entries...
          </div>
        ) : null}
        ...
        {loadError ? (
          ...
          >
            <span role="alert">
              {entries.length > 0
                ? `Memory entries may be stale: ${loadError.message}`
                : `Memory entries could not be loaded: ${loadError.message}`}
            </span>
            <PButton
              type="button"
              compact
        ...
        {parseErrors > 0 ? (
          <p
            data-testid="parse-errors-warning"
            className="rounded-lg border border-warning bg-warning-low p-static-sm text-primary"
          >
            {parseErrors} entries couldn't be read
          </p>
        ) : null}

        {globalMutationMessage ? (
          <p className="rounded-lg border border-success bg-success-low p-static-sm text-primary">
            {globalMutationMessage}
          </p>
        ) : null}
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 flex min-w-0 flex-wrap items-center gap-x-static-sm gap-y-static-xs">
                            ...
                            <span
                              data-testid="memory-entry-category-group"
                            ...
                            >
                              {entry.categories.map((category) => (
                                <PTag
                                  ...
                                  compact
                                  data-testid="memory-entry-category"
                                  variant="secondary"
                                >
                                  {category}
                                </PTag>
                        ...
                        >
                          <PTag
                            compact
                            data-testid="memory-entry-score"
                            variant="secondary"
                            aria-label={`Score: ${formatConfidence(entry.score)}`}
                          >
                      ...
                      {openEntryId === entry.id ? (
                        <div
                          data-testid="memory-accordion-detail"
                          className="grid gap-static-md border-t border-contrast-low pb-static-md pt-static-md"
                        >
                          <div
                          ...
                          <section
                            data-testid="memory-metadata-section"
                            className="rounded-lg border border-contrast-low bg-canvas p-static-sm"
                          >
                            ...
                            >
                              ...
                              {memoryConflict.status === "error" ? (
                                <div className="flex min-w-0 flex-wrap items-center gap-static-xs">
                                  ...
                                  <PButton
                                    type="button"
                                    data-testid="memory-conflict-retry"
                                    compact
                                    variant="secondary"
                                ...
                                <>
                                  <dl className="grid min-w-0 gap-static-sm text-sm sm:grid-cols-2">
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      >
                                        {memoryConflict.currentEntry.content}
                                      </dd>
                                    </div>
                                    <div className="min-w-0">
                          ...
                          {promotionMessageByEntryId[entry.id] ? (
                            <p className="rounded-lg border border-success bg-success-low p-static-sm text-primary">
                              {promotionMessageByEntryId[entry.id]}
                            </p>
                          ) : null}
                          ...
                          >
                            ...
                            {entry.state === "curated" ? (
                              <PButton
                                type="button"
                                data-testid="memory-approve-btn"
                                compact
                                disabled={isMutationPending(entry.id)}
                                aria-busy={isMutationPending(entry.id)}
                          ...
                          {editingEntryId === entry.id && editDraft ? (
                            <div
                              ...
                              className="grid gap-static-md rounded-lg border border-contrast-low bg-canvas p-static-md"
                            >
                              <div
                              ...
                              >
                                ...
                                <div className="flex min-w-0 flex-wrap items-center gap-static-xs">
                                  <PButton
                                    type="button"
                                    data-testid="memory-edit-cancel-btn"
                                    compact
                                    variant="secondary"
                                    disabled={isMutationPending(entry.id)}
                                    onClick={cancelEdit}
                              ...
                              <div className="grid gap-static-sm md:grid-cols-2">
                                <section className="grid min-w-0 gap-static-xs" data-testid="memory-category-editor">
                                  ...
                                  {editDraft.categories.length > 0 ? (
                                    <div
                                      className="flex min-w-0 flex-wrap items-center gap-static-xs"
                                      data-testid="memory-category-chip-list"
                                    >
                                ...
                                <section className="grid min-w-0 gap-static-xs" data-testid="memory-scope-agent-editor">
                                  ...
                                  {editDraft.scope_agents.length > 0 ? (
                                    <div
                                      className="flex min-w-0 flex-wrap items-center gap-static-xs"
                                      data-testid="memory-scope-agent-chip-list"
                                    >

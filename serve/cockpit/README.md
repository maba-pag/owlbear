...
- #1666 is the end-to-end consolidation gate for the Ideas Notebook (all five subtasks: #1660 backend, #1662 core page, #1663 preview toggle, #1664 unsaved-changes guard, #1665 external-edit awareness). `serve/cockpit/web/src/__tests__/IdeasPage.test.tsx` (durable, no task-ID suffix) adds 30 integration tests across 6 describe blocks: `IdeasPageIntegration_SaveFlow` (8 tests), `IdeasPageIntegration_PreviewAfterSave` (4), `IdeasPageIntegration_GuardAfterSave` (3), `IdeasPageIntegration_ConflictTrigger` (5), `IdeasPageIntegration_ConflictOverwrite` (5), and `IdeasPageIntegration_ConflictDiscard` (5). Three cross-feature scenarios unique to this consolidation file: preview renders the saved content after a PUT round-trip (not the original loaded content); navigation guard deactivates after save (save flow + guard interaction); Discard & Reload clears dirty state so navigation proceeds without the alertdialog (conflict resolution + navigation guard interaction). All 30 tests pass against the completed implementation from the dependency tasks with no production-code changes required.

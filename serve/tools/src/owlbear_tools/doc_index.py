...
_AUTO_GENERATED_HEADER = (
    "<!-- AUTO-GENERATED by serve/tools/doc-index."
...
def collect_docs(root: Path) -> list[Path]:
    ...
    for dirpath_str, dirnames, filenames in os.walk(root, topdown=True):
        ...
        dirnames[:] = sorted(
            d for d in dirnames if not _is_excluded_dir(dirpath / d, root)
        )
...
def _update_entry(line: str, entry: DocEntry, *, in_outbound: bool) -> bool:
    ...
    if in_outbound and line.startswith("- ["):
        ...
        if link_m:
            entry["outbound_links"].append(
                LinkEntry(text=link_m.group(1), target=link_m.group(2))
            )

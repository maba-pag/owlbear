...
- [ ] test_list_tasks_success_passes_args in test_server.py passes a valid JSON list (e.g. [{id:1,title:T,status:todo,priority:important,tags:[],class:standard}]) to _patch_run(stdout=...) as a local override, NOT by changing the global _FAKE_STDOUT constant (which other tool tests depend on). The test asserts on the lean JSON success path output (stripped fields absent) instead of raw stdout equality.

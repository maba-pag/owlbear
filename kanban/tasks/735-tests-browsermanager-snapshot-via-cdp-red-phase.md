...
| Mutation reasoning | STRONG | Removing ignored check -> test_excludes_ignored_nodes fails; changing _INTERACTIVE_ROLES -> test_returns_interactive_roles fails; removing finally -> test_detaches_session_on_send_error fails; removing frozen=True -> test_is_frozen fails; removing focusable fallback -> test_focusable_generic_included fails |

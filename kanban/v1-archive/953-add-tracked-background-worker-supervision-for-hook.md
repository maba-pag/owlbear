...
- [ ] RetrospectiveHook must accept an injected HookWorkerSupervisor and use it for background handoff instead of calling asyncio.create_task() directly; the existing success-only, triviality, and priority gating in __call__ and the existing cancel= composition inside _run_retrospective() must remain intact.

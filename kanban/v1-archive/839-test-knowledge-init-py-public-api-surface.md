...
| Test currently FAILS (RED) against 37-symbol __init__ | Standard TDD RED requirement, verifiable | Keep |
...
- When #550 trims __init__.py, TestPhase9Exports in test_knowledge_exports.py will break (asserts removed symbols). That's #550's scope to handle.
...
|---------|---------|----------|
| __all__ defined | test_all_is_defined | happy |
| __all__ is sequence | test_all_is_sequence | happy |
...
### Coverage
- Unable to run scoped `--cov=owlbear.memory.knowledge` (MRO crash). Bare `--cov` confirms knowledge/__init__.py is exercised by all 6 test imports.
...
| 3. test FAILS (RED) | Was RED at test-writer time; builder GREEN-phase trimmed __init__.py (commit 88925e7) | All 6 tests GREEN | PASS |
...
| 3. test FAILS (RED) | Was RED at test-writer; builder GREEN commit 88925e7 trimmed __init__.py | PASS |

"""Failing tests for task #620: pick_tasks MCP tool gate logic and output format.

Covers all AC items from #620:
...
All tests FAIL in RED phase -- pick_tasks does not exist in server.py yet.
"""
...
# ---------------------------------------------------------------------------
# TestFromAC_PickTasksSignature
# ---------------------------------------------------------------------------


class TestFromAC_PickTasksSignature:
    """pick_tasks exists in server.py and exposes the correct callable signature."""

    def test_pick_tasks_is_callable(self) -> None:
        """pick_tasks must be importable and callable."""
        assert callable(pick_tasks)

    def test_has_limit_parameter(self) -> None:
        """pick_tasks must accept a `limit` keyword parameter."""
        sig = inspect.signature(pick_tasks)
        assert "limit" in sig.parameters

    def test_default_limit_is_25(self) -> None:
        """Default value for `limit` is 25 per AC."""
        sig = inspect.signature(pick_tasks)

...
class TestRunCommandSuccess:
    ...
    @pytest.mark.asyncio
    async def test_stderr_empty_on_success(self) -> None:
        ts = TerminalToolset()
...
class TestRunCommandFailure:
    ...
    async def test_nonzero_exit_code(self) -> None:
        ts = TerminalToolset()
        result = await (ts.run_command(f'{PYTHON} -c "import sys; sys.exit(1)"'))
        assert result.exit_code == 1
        assert result.timed_out is False

    @pytest.mark.asyncio
    async def test_nonzero_exit_code_42(self) -> None:
        ts = TerminalToolset()
        result = await (ts.run_command(f'{PYTHON} -c "import sys; sys.exit(42)"'))
...
class TestRunCommandTimeout:
    ...
    @pytest.mark.asyncio
    async def test_timeout_exit_code_nonzero(self) -> None:
...
class TestOutputTruncation:
    ...
    async def test_large_stdout_truncated(self) -> None:
        ...
        result = await (ts.run_command(f"{PYTHON} -c \"print('x' * 500)\""))
    ...
    async def test_small_output_not_truncated(self) -> None:
        ...
        result = await (ts.run_command("echo small"))
    ...
    async def test_truncation_preserves_head_and_tail(self) -> None:
        ...
        result = await (ts.run_command(cmd))
...
class TestHookIntegration:
    ...
    async def test_pre_tool_use_emitted(self) -> None:
        ...
        await (ts.run_command("echo hooked"))
    ...
    @pytest.mark.asyncio
    async def test_no_hooks_skips_emission(self) -> None:
        """When hooks is None, no emission happens (no error)."""
        ts = TerminalToolset(hooks=None)
        result = await (ts.run_command("echo no-hooks"))
        assert result.exit_code == 0

    @pytest.mark.asyncio
    async def test_hook_receives_correct_payload(self) -> None:
        hooks = HookRegistry()
        payloads: list[dict] = []
        hooks.register(HookEvent.PRE_TOOL_USE, payloads.append)

        ts = TerminalToolset(hooks=hooks)
        await (ts.run_command("echo payload-check"))
...
class TestWorkingDir:
    ...
    @pytest.mark.asyncio
    async def test_default_uses_workspace_root(self, tmp_path: Path) -> None:
        ts = TerminalToolset(workspace_root=tmp_path)
        result = await (ts.run_command(f'{PYTHON} -c "import os; print(os.getcwd())"'))
        assert tmp_path.name in result.stdout

    @pytest.mark.asyncio
    async def test_relative_working_dir(self, tmp_path: Path) -> None:
        sub = tmp_path / "subdir"
        sub.mkdir()
        ts = TerminalToolset(workspace_root=tmp_path)
        result = await (
    ...
    @pytest.mark.asyncio
    async def test_absolute_working_dir(self, tmp_path: Path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_none_working_dir_uses_workspace_root(self, tmp_path: Path) -> None:
        ts = TerminalToolset(workspace_root=tmp_path)
        result = await (
            ts.run_command(
                f'{PYTHON} -c "import os; print(os.getcwd())"',
    ...
    @pytest.mark.asyncio
    async def test_working_dir_absolute_outside_raises(self, tmp_path: Path) -> None:
        ts = TerminalToolset(workspace_root=tmp_path)
        outside = "C:\\Windows" if sys.platform == "win32" else "/etc"
        with pytest.raises(PermissionError, match="Path outside workspace"):
            await (ts.run_command("echo hi", working_dir=outside))

    @pytest.mark.asyncio
    async def test_working_dir_traversal_raises(self, tmp_path: Path) -> None:
        ts = TerminalToolset(workspace_root=tmp_path)
        with pytest.raises(PermissionError, match="Path outside workspace"):
            await (ts.run_command("echo hi", working_dir="../../escape"))

    @pytest.mark.asyncio
    async def test_working_dir_null_byte_raises(self, tmp_path: Path) -> None:
        ts = TerminalToolset(workspace_root=tmp_path)
        with pytest.raises(PermissionError, match="Path outside workspace"):
            await (ts.run_command("echo hi", working_dir="sub\x00dir"))

...
class TestFromAC_LintFeedbackStructure:
    ...
    def test_create_file_lint_errors_returns_hook_specific_output(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_replace_string_in_file_lint_errors_returns_hook_specific_output(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_multi_replace_lint_errors_returns_hook_specific_output(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_hook_specific_output_event_name_is_post_tool_use(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_hook_specific_output_additional_context_is_nonempty(
        self, tmp_path: Path
    ) -> None:
        ...
        assert additional_context, (
            f"hookSpecificOutput.additionalContext must be non-empty ruff output, got: {hso!r}"
        )

    # --- AC4 / boundary: systemMessage and additionalContext are identical ---

    def test_system_message_and_additional_context_are_identical(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_lint_errors_output_has_both_system_message_and_hook_specific_output(
        self, tmp_path: Path
    ) -> None:
        ...
        assert "systemMessage" in output, (
            f"Output must contain systemMessage when ruff finds errors. Got: {output!r}"
        )
        assert "hookSpecificOutput" in output, (

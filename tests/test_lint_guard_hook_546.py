...
class TestFromAC_ApplyPatchFilter:
    ...
    def test_apply_patch_in_edit_tools_array(self) -> None:
        ...
        assert _SCRIPT_PATH.exists(), (
            f"lint-changed.ps1 not found at {_SCRIPT_PATH}. "
            "Builder must implement scripts/hooks/lint-changed.ps1."
        )
...
class TestFromAC_ApplyPatchBehavior:
    ...
    def test_apply_patch_clean_file_returns_empty_json(self, tmp_path: Path) -> None:
        ...
        _, output = _run_hook(
            {"tool_name": "apply_patch", "tool_input": {"filePath": str(clean_file)}}
        )
    ...
    def test_apply_patch_lint_errors_returns_system_message(self, tmp_path: Path) -> None:
        ...
        _, output = _run_hook(
            {"tool_name": "apply_patch", "tool_input": {"filePath": str(bad_file)}}
        )
    ...
    def test_apply_patch_lint_errors_returns_hook_specific_additional_context(
        self, tmp_path: Path
    ) -> None:
        ...
        assert "hookSpecificOutput" in output, (
            f"Expected hookSpecificOutput in response, got {output!r}"
        )
        hook_output = output.get("hookSpecificOutput", {})
        assert "additionalContext" in hook_output, (
            f"Expected hookSpecificOutput.additionalContext, got {hook_output!r}"
        )
        assert hook_output["additionalContext"], (
            "hookSpecificOutput.additionalContext must be non-empty ruff output"
        )
    ...
    def test_apply_patch_null_filepath_returns_empty_json(self) -> None:
        ...
        _, output = _run_hook(
            {"tool_name": "apply_patch", "tool_input": {"filePath": None}}
        )
        assert output == {}, f"Expected {{}}, got {output!r}"
    ...
    def test_apply_patch_exit_code_never_2_lint_errors(self, tmp_path: Path) -> None:
        ...
        assert code != 2, (
            f"Lint errors must not block builder via exit code 2, got {code}"
        )
    ...
    def test_apply_patch_exit_code_never_2_missing_filepath(self) -> None:
        ...
        assert code != 2, (
            f"Script must never exit with code 2 for missing filePath, got {code}"
        )
    ...
    def test_apply_patch_stdout_is_valid_json_for_lint_errors(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            pytest.fail(
                f"Script stdout is not valid JSON for apply_patch: {stdout!r}\nError: {exc}"
            )

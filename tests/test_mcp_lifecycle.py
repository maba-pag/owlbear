...
class TestFromAC_MoveTaskGuidanceViaMock:
    """``move_task`` returns non-empty guidance when the fallback path is active.

    All tests force ``engine.agent_view = None`` so the L481 code path runs.
    They fail because the stale dict-form statuses cause ``collect_guidance``
    to receive ``list[dict]`` for ``status_names``, which silently returns
    ``[]`` via a suppressed ``ValueError``.  After the builder fixes the mock,
    ``collect_guidance`` receives ``list[str]`` and computes the skip correctly.
    """

    @pytest.mark.asyncio
    async def test_forward_skip_more_than_one_slot_returns_guidance(self) -> None:
        """move_task forward-skip >1 slot returns non-empty guidance (mock fallback path).

        Task moves from 'research' to 'todo' (skipping 'backlog' — 2-slot jump).

        FAILS: dict-form statuses → ``status_names.index("research")`` raises
        ``ValueError`` inside ``_move_guidance`` → guidance stays ``[]``.
        ...
        assert len(result.guidance) > 0, (
            ...
            f"Likely cause: stale dict-form statuses in test_mcp_lifecycle_tools."
        )

    @pytest.mark.asyncio
    async def test_forward_skip_guidance_message_references_source_status(self) -> None:
        """Forward-skip guidance message contains the source status name ('research').

        FAILS: guidance is ``[]`` due to stale dict-form mock (same root cause).
        PASSES: after builder fixes mock to list[str].
        """
        ctx = _make_fallback_ctx(before_dict=_RESEARCH_TASK, after_dict=_TODO_TASK)
        result = await move_task(ctx, id="1", status="todo")
    ...
    async def test_forward_skip_guidance_message_references_target_status(self) -> None:
        """Forward-skip guidance message contains the target status name ('todo').

...
class TestFromAC_CockpitPackageSkeleton:
    ...
    def test_pyproject_toml_declares_owlbear_delivery_dep(self, project_root: Path) -> None:
        """AC#1: pyproject.toml lists owlbear-delivery as a dependency."""
        content = (project_root / "serve" / "cockpit" / "pyproject.toml").read_text()
        assert "owlbear-delivery" in content
    ...
    def test_pyproject_toml_has_workspace_source_for_delivery(self, project_root: Path) -> None:
        """AC#1: pyproject.toml wires owlbear-delivery as a uv workspace source."""
        content = (project_root / "serve" / "cockpit" / "pyproject.toml").read_text()
        assert "workspace = true" in content, (
            "owlbear-delivery must be declared as workspace source "
            "([tool.uv.sources] owlbear-delivery = { workspace = true })"
        )
...
class TestFromAC_BoundaryEnforcement:
    ...
    claim_task, start_work, end_work, pick_dispatchable from owlbear_delivery*.
    """

...
def _make_board(tmp_path: Path, config_yaml: str = _FLAT_CONFIG_YAML) -> Path:
...
class TestFromAC_BoardConfigRootExtraAllow:
    ...
    def test_grouped_config_preserves_vendor_field_at_root(self, tmp_path: Path) -> None:
        ...
        yaml_with_vendor = _GROUPED_CONFIG_YAML + "vendor_integration: true\n"
    ...
    def test_grouped_config_preserves_tui_section(self, tmp_path: Path) -> None:
        ...
        yaml_with_tui = _GROUPED_CONFIG_YAML + "tui:\n  theme: dark\n"
...
class TestFromAC_DetectionCascade:
    ...
    def test_cascade_flat_keyset_loads_without_sub_model_sections(
        ...
        kanban_dir = _make_board(tmp_path, _FLAT_CONFIG_YAML)
        config = load_config(kanban_dir)
        # tasks_dir accessible via forwarding property even for flat format
    ...
    def test_cascade_legacy_keys_load_correctly(self, tmp_path: Path) -> None:
        ...
        config = load_config(kanban_dir)
        assert config.statuses == ["research", "done"]
    ...
    def test_cascade_grouped_takes_precedence_over_flat_detection(
        ...
        kanban_dir = _make_board(tmp_path, _GROUPED_CONFIG_YAML)
        config = load_config(kanban_dir)
        # paths.tasks_dir comes from the paths: section, not a flat tasks_dir key
...
class TestFromAC_ForwardingProperties:
    ...
    def test_forwarding_property_values_match_yaml(self, tmp_path: Path) -> None:
        """Forwarding properties return values from the paths: section in config.yml."""
        kanban_dir = _make_board(tmp_path, _GROUPED_CONFIG_YAML)
        config = load_config(kanban_dir)
        assert config.tasks_dir == "tasks"
...
"""
        ...
        config = load_config(kanban_dir)
        # Must be "someday", not the PipelineConfig model default
...
class TestFromAC_SaveConfigGroupedFormat:
    ...
    def test_save_config_emits_paths_section(self, tmp_path: Path) -> None:
        ...
        assert "tasks_dir:" in written
        assert "archive_dir:" in written

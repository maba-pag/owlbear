...
class TestAddEvidence:
    ...
    def test_add_entity_evidence_returns_evidence_record(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_add_edge_evidence_returns_evidence_record(self, store: SqliteGraphStore, edge_ab) -> None:
    ...
    def test_add_evidence_idempotent_returns_same_id(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_add_evidence_confidence_zero_accepted(self, store: SqliteGraphStore, entity_a) -> None:
        """AC1 boundary: confidence=0.0 is valid lower bound."""
        result = store.add_evidence(_entity_evidence("chunk-conf0", entity_a.id, confidence=0.0))
        assert result.confidence == 0.0

    def test_add_evidence_confidence_one_accepted(self, store: SqliteGraphStore, entity_a) -> None:
        """AC1 boundary: confidence=1.0 is valid upper bound."""
        result = store.add_evidence(_entity_evidence("chunk-conf1", entity_a.id, confidence=1.0))
...
class TestClaimsForChunk:
    """AC2: claims_for_chunk returns ChunkClaims; empty for unknown chunk_id."""

    # ------------------------------------------------------------------ happy

    def test_claims_for_chunk_returns_chunk_claims_instance(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_claims_for_chunk_includes_entity_id(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_claims_for_chunk_includes_evidence_ids(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_claims_for_chunk_empty_for_unknown_chunk(self, store: SqliteGraphStore) -> None:
    ...
    def test_claims_for_chunk_includes_edge_id(self, store: SqliteGraphStore, edge_ab) -> None:
    ...
    def test_claims_for_chunk_mixed_entity_and_edge_claims(self, store: SqliteGraphStore, entity_a, edge_ab) -> None:
...
class TestInvalidateEvidenceByChunks:
    """AC3: invalidate_evidence_by_chunks hard-deletes; orphan cascade; idempotent."""

    # ------------------------------------------------------------------ happy

    def test_invalidate_returns_evidence_invalidation_result(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_invalidate_reports_invalidated_evidence_ids(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_invalidate_orphans_entity_with_no_remaining_evidence(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_invalidate_orphaned_entity_is_deleted_from_graph(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_invalidate_cascades_aliases_of_orphaned_entity(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_invalidate_idempotent_for_absent_chunk_ids(self, store: SqliteGraphStore) -> None:
    ...
    def test_invalidate_empty_chunk_ids_is_no_op(self, store: SqliteGraphStore) -> None:
...
class TestAddAlias:
    ...
    def test_add_alias_returns_entity_alias_record(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_add_alias_stores_canonical_form(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_add_alias_find_entities_resolves_alias_transparently(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_add_alias_idempotent_for_same_entity_and_alias(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_add_alias_raises_lookup_error_for_missing_entity(self, store: SqliteGraphStore) -> None:
        """AC4 error: LookupError if entity_id does not exist."""
        with pytest.raises(LookupError):
            store.add_alias(EntityAliasInput(entity_id="nonexistent-entity-id", alias_name="Orphan-Alias"))
...
class TestTraverse:
    ...
    def test_traverse_includes_directly_connected_entity(self, store: SqliteGraphStore, entity_a, entity_b) -> None:
    ...
    def test_traverse_includes_directly_connected_edge(self, store: SqliteGraphStore, entity_a, edge_ab) -> None:
        """AC5 happy: connecting edge appears in traversal edges."""
        result = store.traverse(TraversalQuery(entity_id=entity_a.id, max_hops=1))
        edge_ids = {e.id for e in result.edges}
        assert edge_ab.id in edge_ids

    def test_traverse_multi_hop_reaches_distant_entity(self, store: SqliteGraphStore, entity_a, entity_b) -> None:
    ...
    def test_traverse_total_size_respects_limit(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_traverse_no_duplicate_entities_in_result(self, store: SqliteGraphStore, entity_a, entity_b) -> None:
    ...
    def test_traverse_raises_lookup_error_for_nonexistent_seed(self, store: SqliteGraphStore) -> None:
    ...
    def test_traverse_seed_with_no_edges_returns_no_edges(self, store: SqliteGraphStore, entity_a) -> None:
...
class TestStats:
    ...
    def test_stats_reflects_evidence_count(self, store: SqliteGraphStore, entity_a) -> None:
    ...
    def test_stats_reflects_alias_count(self, store: SqliteGraphStore, entity_a) -> None:
...
class TestEnsureTablesExtended:
    """AC7: ensure_tables creates graph_evidence + graph_aliases; idempotent."""

    def test_ensure_tables_creates_graph_evidence_table(self, db: sqlite3.Connection) -> None:
        """AC7 happy: graph_evidence table exists after ensure_tables."""
        store = SqliteGraphStore(db)
        store.ensure_tables()
        row = db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='graph_evidence'").fetchone()
        assert row is not None, "graph_evidence table was not created by ensure_tables"

    def test_ensure_tables_creates_graph_aliases_table(self, db: sqlite3.Connection) -> None:
        """AC7 happy: graph_aliases table exists after ensure_tables."""
        store = SqliteGraphStore(db)
        store.ensure_tables()
        row = db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='graph_aliases'").fetchone()
        assert row is not None, "graph_aliases table was not created by ensure_tables"

    def test_ensure_tables_idempotent_creates_all_tables_on_second_call(self, db: sqlite3.Connection) -> None:
    ...
    def test_ensure_tables_graph_evidence_has_index_on_chunk_id(self, db: sqlite3.Connection) -> None:
        ...
        for (idx_name,) in indexes:
            ...
            if any(col[2] == "chunk_id" for col in cols):
                ...
                break
        assert chunk_id_indexed, f"No index on chunk_id found in graph_evidence. Indexes: {[r[0] for r in indexes]}"

    def test_ensure_tables_graph_aliases_unique_constraint_rejects_duplicate(self, db: sqlite3.Connection) -> None:
        ...
        db.execute(
            "INSERT INTO graph_aliases (id, entity_id, alias_name, canonical_alias, created_at) VALUES (?, ?, ?, ?, ?)",
...
class TestAddEvidence_XorAndUpdate:
    ...
    def test_add_evidence_repeat_call_updates_confidence_mutable_field(self, store: SqliteGraphStore, entity_a) -> None:
...
class TestStats_ExactCounts:
    ...
    def test_stats_exact_entity_count(self, db: sqlite3.Connection) -> None:
        ...
        result = store.stats()
        assert result.entities == 2, f"expected exactly 2 entities, got {result.entities}"
    ...
    def test_stats_exact_edge_count(self, db: sqlite3.Connection) -> None:
        ...
        store.upsert_edge(_mk_edge(a.id, b.id))
        result = store.stats()
        assert result.edges == 1, f"expected exactly 1 edge, got {result.edges}"
    ...
    def test_stats_exact_evidence_count(self, db: sqlite3.Connection) -> None:
        ...
        result = store.stats()
        assert result.evidence_claims == 1, f"expected exactly 1 evidence_claim, got {result.evidence_claims}"
    ...
    def test_stats_exact_alias_count(self, db: sqlite3.Connection) -> None:
        ...
        result = store.stats()
        assert result.aliases == 1, f"expected exactly 1 alias, got {result.aliases}"


# Finding #3 — AC7: missing index and FK coverage


class TestEnsureTablesExtended_IndexAndFK:
    ...
    def test_ensure_tables_graph_evidence_has_index_on_entity_id(self, db: sqlite3.Connection) -> None:
        ...
        for (idx_name,) in indexes:
            ...
            if any(col[2] == "entity_id" for col in cols):
                ...
                break
        assert entity_id_indexed, f"No index on entity_id found in graph_evidence. Indexes: {[r[0] for r in indexes]}"

    def test_ensure_tables_graph_evidence_has_index_on_edge_id(self, db: sqlite3.Connection) -> None:
        ...
        for (idx_name,) in indexes:
            ...
            if any(col[2] == "edge_id" for col in cols):
                ...
                break
        assert edge_id_indexed, f"No index on edge_id found in graph_evidence. Indexes: {[r[0] for r in indexes]}"

    def test_ensure_tables_graph_aliases_has_index_on_canonical_alias(self, db: sqlite3.Connection) -> None:
        ...
        assert canonical_alias_indexed, (
            f"No index on canonical_alias found in graph_aliases. Indexes: {[r[0] for r in indexes]}"
        )

    def test_ensure_tables_graph_evidence_fk_entity_id_references_graph_entities(self, db: sqlite3.Connection) -> None:
    ...
    def test_ensure_tables_graph_evidence_fk_edge_id_references_graph_edges(self, db: sqlite3.Connection) -> None:
    ...
    def test_ensure_tables_graph_aliases_fk_entity_id_references_graph_entities(self, db: sqlite3.Connection) -> None:
...
class TestClaimsForChunk_Exclusivity:
    ...
    def test_claims_for_chunk_excludes_edge_ids_from_other_chunk(
        ...
        store.add_evidence(_entity_evidence("chunk-B", entity_b.id))
        result = store.claims_for_chunk("chunk-A")
        assert edge_ab.id not in result.edge_ids, "edge linked only via chunk-B must not appear in claims for chunk-A"
    ...
    def test_claims_for_chunk_excludes_evidence_ids_from_other_chunk(
        ...
        result = store.claims_for_chunk("chunk-A")
        assert ev_b.id not in result.evidence_ids, "evidence record from chunk-B must not appear in claims for chunk-A"

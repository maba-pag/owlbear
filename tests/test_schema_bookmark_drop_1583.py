...
class TestFromAC_SchemaBookmarkConsolidationDrop:
    ...
    def test_ac2_init_db_no_bookmark_indexes(self) -> None:
        ...
        try:
            ...
            assert "idx_bookmarks_scope" not in index_names, (
                "idx_bookmarks_scope must not be created by init_db"
            )
        finally:
            conn.close()
    ...
    def test_ac3_migration_v11_to_v12_drops_tables(self) -> None:
        assert hasattr(schema_mod, "_migrate_v11_to_v12"), (
            "_migrate_v11_to_v12 must exist in schema module"
        )
        conn = sqlite3.connect(":memory:")
        try:
            ...
            assert "consolidations" not in tables, (
                "consolidations must be dropped by v12 migration"
            )
        finally:
            conn.close()
    ...
    def test_ac4_schema_version_is_12(self) -> None:
        assert schema_mod._SCHEMA_VERSION == 12, (
            f"_SCHEMA_VERSION must be 12, got {schema_mod._SCHEMA_VERSION}"
        )
    ...
    def test_ac5_docstring_excludes_retired_tables(self) -> None:
        ...
        assert "bookmarks" not in doc, (
            "Module docstring must not mention bookmarks"
        )
        assert "consolidations" not in doc, (
            "Module docstring must not mention consolidations"
        )
    ...
    def test_ac6_fresh_db_no_retired_tables(self) -> None:
        ...
        try:
            ...
            assert "bookmarks" not in tables, (
                "init_db must not create bookmarks table in fresh database"
            )
            assert "consolidations" not in tables, (
                "init_db must not create consolidations table in fresh database"
            )
        finally:
            conn.close()
    ...
    def test_ac7_migration_from_v11_drops_retired_tables(self) -> None:
        ...
        try:
            ...
            conn.execute(
                ...
                "id TEXT PRIMARY KEY, source_ids TEXT NOT NULL"
                ")"
            )
            ...
            assert "bookmarks" not in tables, (
                "_apply_migrations from v11 must drop the bookmarks table"
            )
            assert "consolidations" not in tables, (
                "_apply_migrations from v11 must drop the consolidations table"
            )
        finally:
            conn.close()

...
class TestFromAC_StructuredJsonErrors:
    ...
    def test_map_kanban_error_emits_parseable_json(self) -> None:
        ...
        with pytest.raises(ToolError) as exc_info:
            _map_kanban_error(NotFoundError(code="ERR_NOT_FOUND", user_message="task 99 not found"))
    ...
    def test_map_kanban_error_json_contains_message_field(self) -> None:
        ...
        with pytest.raises(ToolError) as exc_info:
            _map_kanban_error(ValidationError(code="ERR_INVALID_STATUS", user_message="invalid status"))
        payload = json.loads(str(exc_info.value))
    ...
    def test_map_kanban_error_json_message_equals_user_message(self) -> None:
        ...
        with pytest.raises(ToolError) as exc_info:
            _map_kanban_error(NotFoundError(code="ERR_NOT_FOUND", user_message=user_msg))
        payload = json.loads(str(exc_info.value))
    ...
    def test_map_kanban_error_err_stale_code_in_json(self) -> None:
        ...
        payload = json.loads(str(exc_info.value))
        assert payload["code"] == "ERR_STALE", "ERR_STALE must appear as 'code' in JSON for agent retry routing"
    ...
    def test_map_kanban_error_err_not_found_stop_signal(self) -> None:
        ...
        with pytest.raises(ToolError) as exc_info:
            _map_kanban_error(NotFoundError(code="ERR_NOT_FOUND", user_message="task 999 not found"))
        payload = json.loads(str(exc_info.value))
        assert payload["code"] == "ERR_NOT_FOUND"
    ...
    def test_map_kanban_error_message_is_json_object_not_bare_string(self) -> None:
        ...
        assert err_str.startswith("{"), "ToolError message must be a JSON object (starts with '{'), not a bare string"
    ...
    def test_agent_can_parse_error_code_from_tool_error_text(self) -> None:
        ...
        with pytest.raises(ToolError) as exc_info:
            _map_kanban_error(NotFoundError(code="ERR_NOT_FOUND", user_message="task 1 not found"))
        ...
        code = parsed["code"]
        assert code == "ERR_NOT_FOUND", "agent must extract error code from JSON to determine routing (stop/retry/etc.)"
    ...
    def test_map_kanban_error_user_message_with_special_chars_round_trips(self) -> None:
        ...
        with pytest.raises(ToolError) as exc_info:
            _map_kanban_error(NotFoundError(code="ERR_NOT_FOUND", user_message=user_msg))
        payload = json.loads(str(exc_info.value))
        assert payload["message"] == user_msg, "JSON encoding must preserve user_message including quotes and slashes"

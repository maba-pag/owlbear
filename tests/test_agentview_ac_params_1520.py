...
AC coverage:
  AC1 → TestFromAC_CreateTaskAcProofBundle — create_task accepts ac/proof_bundle;
         returned SingleTaskResponse.task.ac and .task.proof_bundle reflect provided values
...
class TestFromAC_CreateTaskAcProofBundle:
    ...
    def test_create_task_ac_and_proof_bundle_in_returned_task(self, tmp_path: Path) -> None:
        ...
        assert result.task.ac == ["x"]
        assert result.task.proof_bundle == "behavioral"
    ...
    def test_create_task_ac_param_only(self, tmp_path: Path) -> None:
        ...
        assert result.task.ac == ["criterion-a"]
        assert result.task.proof_bundle is None
    ...
    def test_create_task_multiple_ac_items_all_preserved(self, tmp_path: Path) -> None:
        ...
        assert result.task.ac == ac_items

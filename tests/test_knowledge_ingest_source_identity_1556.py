...
class TestFromAC_ReplaceOnChangeFailureCleanup:
    ...
    async def test_replace_on_change_failure_preserves_original_document(
        ...
        # content differs. The replacement is staged first; on failure, v2 is
        # cleaned up and the last good v1 document remains.
        with patch.object(
            store_components["doc_store"],
            "store_chunks",
    ...
    async def test_replace_on_change_late_failure_leaves_no_chunks_or_vectors(
        ...
        """When store_extractions raises AFTER insert_document(v2) +
        store_chunks(v2) + store_embeddings(v2) have all
        ...
        AC-8 contract: persistence preserves the last good document and contains no
        document, chunk, vector payload, entity, or edge rows for the replacement doc_id.
        """
        self._seed_url_list_source(conn)
        pipeline: IngestPipeline = store_components["pipeline"]
        ...
        # (a) v1 remains and v2 is cleaned by _cleanup_failed_ingest.
        doc_count = conn.execute("SELECT count(*) FROM documents").fetchone()[0]
        replacement_count = conn.execute(
            "SELECT count(*) FROM documents WHERE id = ?",
            (result_v2.document_id,),
        ...
        # (b) v2 chunks committed by store_chunks must be cleaned up.
        ...
        # (d) no v2 entities or edges survive.
        ...
        edge_count = conn.execute(
            "SELECT count(*) FROM edges WHERE document_id = ?",
            (result_v2.document_id,),
        ).fetchone()[0]
        assert edge_count == 0
    ...
    async def test_replace_on_change_v2_chunk_ids_specifically_deleted_from_vector_store(
        ...
        # Existing AC-8 contract: no replacement rows survive after failed replace-on-change.
        doc_count = conn.execute("SELECT count(*) FROM documents").fetchone()[0]
        replacement_doc_count = conn.execute(
            "SELECT count(*) FROM documents WHERE id = ?",
            (result_v2.document_id,),
        ).fetchone()[0]
        assert doc_count == 1
        assert replacement_doc_count == 0
        chunk_count = conn.execute(
            "SELECT count(*) FROM chunks WHERE document_id = ?",
            (result_v2.document_id,),
        ).fetchone()[0]

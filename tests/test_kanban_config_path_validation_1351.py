...
class TestFromAC_PathsConfigValidation:
    ...
    def test_empty_string_tasks_dir_is_boundary(self) -> None:
        """Empty tasks_dir resolves to kanban_dir itself — caught at config or runtime."""
        from owlbear_kanban._naming import validate_config_path_containment  # noqa: PLC0415

        # Empty is a boundary: not absolute, no '..' component, but resolves to parent dir.
        # Builder may reject at config-time or leave to runtime validate_path_containment.
        try:
            validate_config_path_containment("")
        except ConfigError:
            pass  # config-time rejection is acceptable
        else:
            # If not rejected at config, runtime must catch it (path equals kanban_dir).
            pytest.skip("empty string accepted at config-time; runtime is the backstop")
...
class TestFromAC_RuntimeDefense:
    ...
    def test_symlink_pointing_outside_rejected_at_runtime(self, tmp_path: Path) -> None:
        ...
        # Runtime resolve-level check must catch the escape
        with pytest.raises(PermissionError):
            validate_path_containment(kanban_dir, outside_file)

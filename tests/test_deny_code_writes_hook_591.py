...
def _extract_frontmatter(content: str) -> str:
    ...
    match = re.match(r"^---\n(.*?)\n---", content, re.DOTALL)
    assert match is not None, "No valid YAML frontmatter (--- ... ---) found in doc-writer.agent.md"
...
class TestFromAC_ScriptExists:
    ...
    def test_script_is_nonempty(self) -> None:
        ...
        assert _SCRIPT_PATH.exists(), f"deny-code-writes.ps1 not found at {_SCRIPT_PATH}."
        assert _SCRIPT_PATH.stat().st_size > 0, "deny-code-writes.ps1 exists but is empty -- builder must implement it."
...
class TestFromAC_DenyListBehavior:
    ...
    def test_create_file_serve_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with serve/ path must be denied (deny-list), got: {output!r}"
    ...
    def test_create_file_v1_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with v1/ path must be denied (deny-list), got: {output!r}"
    ...
    def test_create_file_tests_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with tests/ path must be denied (deny-list), got: {output!r}"
    ...
    def test_create_file_setup_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with setup/ path must be denied (deny-list), got: {output!r}"
    ...
    def test_create_file_seed_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with seed/ path must be denied (deny-list), got: {output!r}"
    ...
    def test_create_file_store_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with store/ path must be denied (deny-list), got: {output!r}"
    ...
    def test_create_file_git_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with .git/ path must be denied (privilege escalation), got: {output!r}"
    ...
    def test_create_file_owlbear_scripts_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with .owlbear/scripts/ path must be denied, got: {output!r}"
    ...
    def test_create_file_conftest_py_exact_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_file with exact path 'conftest.py' must be denied, got: {output!r}"
    ...
    def test_create_file_readme_is_allowed(self) -> None:
        ...
        assert output == {}, f"create_file with README.md must be allowed (not in deny-list), got: {output!r}"
    ...
    def test_create_file_github_copilot_instructions_is_allowed(self) -> None:
        ...
        assert output == {}, f"create_file with .github/copilot-instructions.md must be allowed, got: {output!r}"
    ...
    def test_create_file_share_skills_is_allowed(self) -> None:
        ...
        assert output == {}, f"create_file with share/skills/ path must be allowed, got: {output!r}"
    ...
    def test_create_file_owlbear_research_is_allowed(self) -> None:
        ...
        assert output == {}, f"create_file with .owlbear/research/ path must be allowed, got: {output!r}"
    ...
    def test_backslash_serve_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"Backslash serve\\ path must be denied after normalization, got: {output!r}"
    ...
    def test_backslash_share_skills_is_allowed(self) -> None:
        ...
        assert output == {}, f"Backslash share\\skills\\ path should be normalized and allowed, got: {output!r}"
    ...
    def test_dot_slash_serve_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"./serve/ path must be denied after ./ stripping, got: {output!r}"
    ...
    def test_dot_slash_conftest_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"./conftest.py must be denied after ./ stripping (exact match), got: {output!r}"
    ...
    def test_replace_string_in_file_v1_path_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"replace_string_in_file with v1/ path must be denied, got: {output!r}"
    ...
    def test_replace_string_in_file_allowed_path_returns_empty(self) -> None:
        ...
        assert output == {}, f"replace_string_in_file with non-denied path must return {{}}, got: {output!r}"
    ...
    def test_multi_replace_all_denied_paths_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"multi_replace with all denied paths must be denied, got: {output!r}"
    ...
    def test_multi_replace_mixed_paths_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"multi_replace with any denied path must deny the whole call, got: {output!r}"
    ...
    def test_multi_replace_all_allowed_paths_returns_empty(self) -> None:
        ...
        assert output == {}, f"multi_replace with all allowed paths must return {{}}, got: {output!r}"
    ...
    def test_create_directory_tests_dirpath_is_denied(self) -> None:
        ...
        assert _is_denied(output), f"create_directory with tests/ dirPath must be denied, got: {output!r}"
    ...
    def test_create_directory_allowed_dirpath_returns_empty(self) -> None:
        ...
        assert output == {}, f"create_directory with non-denied dirPath must return {{}}, got: {output!r}"
    ...
    def test_conftest_py_bak_is_allowed(self) -> None:
        ...
        assert output == {}, f"'conftest.py.bak' must be allowed (not exact match for 'conftest.py'), got: {output!r}"
    ...
    def test_seedfile_py_not_seed_dir_is_allowed(self) -> None:
        ...
        assert output == {}, f"'seedfile.py' must be allowed ('seed' but not 'seed/'), got: {output!r}"
    ...
    def test_share_skills_is_allowed_but_share_agents_is_denied(self) -> None:
        ...
        assert _is_denied(denied_output), f"share/agents/ must be denied (self-modification), got: {denied_output!r}"
        ...
        assert allowed_output == {}, f"share/skills/ must be allowed (not in deny-list), got: {allowed_output!r}"
    ...
    def test_deny_response_permission_decision_is_deny(self) -> None:
        ...
        assert decision == "deny", f"permissionDecision must equal 'deny', got: {decision!r} in {output!r}"
    ...
    def test_stdout_is_valid_json_for_allowed_call(self) -> None:
        ...
        if not _SCRIPT_PATH.exists():
            raise FileNotFoundError(f"deny-code-writes.ps1 not found at {_SCRIPT_PATH}.")
        result = subprocess.run(
            ["powershell", "-NoProfile", "-NonInteractive", "-File", str(_SCRIPT_PATH)],
...
class TestFromAC_WriteToolGate:
    ...
    def test_create_file_is_gated(self) -> None:
        ...
        assert _is_denied(output), f"create_file must be in the write-tool gate, got: {output!r}"
    ...
    def test_replace_string_in_file_is_gated(self) -> None:
        ...
        assert _is_denied(output), f"replace_string_in_file must be in the write-tool gate, got: {output!r}"
    ...
    def test_multi_replace_string_in_file_is_gated(self) -> None:
        ...
        assert _is_denied(output), f"multi_replace_string_in_file must be in the write-tool gate, got: {output!r}"
    ...
    def test_create_directory_is_gated(self) -> None:
        ...
        assert _is_denied(output), f"create_directory must be in the write-tool gate, got: {output!r}"
    ...
    def test_run_in_terminal_returns_empty_json(self) -> None:
        ...
        assert output == {}, f"run_in_terminal must pass through (not gated), got: {output!r}"
    ...
    def test_read_file_returns_empty_json(self) -> None:
        ...
        assert output == {}, f"read_file must pass through (not gated), got: {output!r}"
    ...
    def test_unknown_tool_name_returns_empty_json(self) -> None:
        ...
        assert output == {}, f"Unknown tool must pass through, got: {output!r}"
...
class TestFromAC_SafetyFallbacks:
    ...
    def test_empty_tool_name_returns_empty_json(self) -> None:
        ...
        assert output == {}, f"Empty tool_name must return {{}}, got: {output!r}"
    ...
    def test_missing_tool_name_key_returns_empty_json(self) -> None:
        ...
        assert output == {}, f"Missing tool_name key must return {{}}, got: {output!r}"
...
class TestFromAC_DocWriterAgentHooks:
    ...
    def test_frontmatter_has_hooks_section(self) -> None:
        ...
        assert re.search(r"^hooks:", fm, re.MULTILINE), (
            "doc-writer.agent.md frontmatter is missing the 'hooks:' key. Builder must add a PreToolUse hooks section."
        )

    def test_frontmatter_has_pretooluse_entry(self) -> None:
        """AC3b: hooks: section must contain a PreToolUse entry."""
        fm = self._frontmatter()
        assert "PreToolUse" in fm, "doc-writer.agent.md hooks: section is missing a PreToolUse entry."

    def test_pretooluse_hook_type_is_command(self) -> None:
        """AC3c: PreToolUse hook must specify type: command."""
        fm = self._frontmatter()
        assert re.search(r"type:\s*command", fm), "doc-writer.agent.md PreToolUse hook must specify 'type: command'."
...
class TestFromAC_MaintenanceHeader:
    ...
    def test_script_header_mentions_denied_dirs(self) -> None:
        ...
        denied_mentioned = any(d in content for d in ["serve/", "share/agents/", ".git/", "deny-list"])

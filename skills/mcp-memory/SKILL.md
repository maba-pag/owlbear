---
name: mcp-memory
description: "Query, record, and manage agent institutional knowledge via the owlbearMemory MCP server. Covers all 5 tools: get_knowledge, record_learning, list_entries, set_approval_state, mark_for_deletion."
user-invocable: false
---

# MCP Memory Skill

The `owlbearMemory` MCP server exposes agent memory operations as MCP tools over stdio transport.

## Server registration

The server is registered in `.vscode/mcp.json` as `owlbearMemory`.
...
### Step 0 — Institutional knowledge pre-flight

Called at the start of every task to load accumulated agent learnings:

```python
# Load up to 20 entries with confidence >= 0.7
entries = get_knowledge(agent_id="builder", limit=20, min_confidence=0.7)
# Apply returned patterns, pitfalls, and workarounds as context
```

If the call fails or returns empty, proceed normally — graceful degradation.

### Post-task reflection — dual-write pattern

One call per notable finding, with matching category:

```python
# Pattern discovered
record_learning(
    agent_id="builder",
    content="Use bare --cov flag for scoped pytest coverage; --cov=path causes false negatives",
    category="behavior",
    confidence=0.8,
    scope_agent="builder",
)

# Problem + workaround pair
record_learning(
    agent_id="builder",
    content="PS 5.1 here-strings split args in Start-Process ArgumentList — use temp file pattern instead",
    category="knowledge",
    confidence=0.8,
    scope_agent="builder",
)
...
If `record_learning` returns `error: ...`, log it and continue — the `memory create` fallback still captures the data.

### Curator pattern — review and approve pending entries

```python
# List pending entries for triage
pending = list_entries(status="pending")

# Approve a high-signal entry
set_approval_state(entry_id="<uuid>", new_state="approved")

# Soft-delete a low-signal entry
mark_for_deletion(entry_id="<uuid>")
```

...
> **MCP equivalent:** `start_work` (Phase 1+2 combined), `edit_task` (Phase 2 body edits), `end_work` (Phase 3 advance + release).
...
- **`--claim` and `--release` must be separate calls.** Combining them in one command (e.g., `edit --status next --claim X --release`) errors or silently ignores one flag. Always issue a separate `edit --release` call after the status-move.
- **Body-append requires `--claim <agent>` when task is already claimed by you.** Omitting `--claim` on `edit -a` while the task is yours produces a "claimed by another agent" conflict error. Always include `--claim <your-agent>` on append calls.
- **Task files must have LF line endings.** Python `write_text()` on Windows writes CRLF, causing "file does not start with YAML frontmatter" errors. Use `write_bytes(content.encode("utf-8").replace(b"\r\n", b"\n"))` when writing task files via Python.

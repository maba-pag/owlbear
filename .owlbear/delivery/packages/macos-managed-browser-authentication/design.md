...
The revised managed-device procedure is maintained in `setup/setup-guide.md` under the managed-Mac browser pilot section. It instructs the operator to supply one approved SharePoint URL and one approved Confluence URL locally, confirm that authenticated access is available, open both targets in the lifespan-owned Edge page, and record only bounded target-class and pass/fail outcomes. The procedure verifies no rejected sandbox warning, restarts the profile, and verifies session availability. It retains no page content, target URLs, hostnames, credentials, or session files in Delivery authority or browser status. Prior SharePoint/Jira pilot observations remain historical evidence for launcher/profile behavior but do not satisfy this revised target set.
...
acceptance:
  ...
  - "Given the revised managed-Mac pilot procedure, the lifespan-owned Edge page opens operator-approved SharePoint and Confluence targets with operator-confirmed authenticated content, retains session availability after profile restart, emits no rejected no-sandbox warning, and closes only OwlBear-owned resources."

...
- [ ] generate() raises ValueError when diagram_type in _SVG_ONLY_TYPES and output_format != "svg" -- check placed after existing output_format guard (service.py ~L49)
...
- Evidence: Added _SVG_ONLY_TYPES=frozenset({excalidraw}); moved SVG-only ValueError guard to run after output_format validation; guard now checks diagram_type in _SVG_ONLY_TYPES

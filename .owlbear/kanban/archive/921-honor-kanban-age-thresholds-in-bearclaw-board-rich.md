...
| AC1: load tui.age_thresholds task-local only | covered by _with_config context manager for all 9 tests | happy |
...
|---------|----------|-------------|--------|
| AC1 | _load_board_config at board.py line 30 and _read_age_thresholds at line 103 keep the config handling local to board.py; the builder commit touched only board.py and imports remain existing yaml and Rich modules | Missing-config and malformed-config fallback tests plus the positive threshold-style tests | PASS |
...
| AC4: fallback on invalid config | OSError/YAMLError caught in _load_board_config, shape/key/duration/color validation in _read_age_thresholds returns [] | PASS |

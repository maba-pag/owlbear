...
**Final:** "_migrate_config produces grouped output — no flat duplicate keys at root; activity_log not in _LEGACY_CONFIG_KEYS so grouped configs are correctly recognized as already migrated; returns 'already' for already-grouped configs without writing" (td:2)
...
| AC9 (FINAL): grouped output, idempotency | TestFromAC_MigrateConfigCleanGroupedOutput + TestFromAC_MigrateIdempotencyAndPreservation green; activity_log removed from _LEGACY_CONFIG_KEYS | PASS |

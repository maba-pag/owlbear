...
| Test: HookReactionRule declared in config.py + order preservation | 4 tests (class ClassDef, __module__, rule order, action order) all pass; HookReactionRule at config.py L36 | PASS |
| Test: Router consumes config-owned rules, no duplicate | 3 tests (no ClassDef in router, config __module__, e2e dispatch) all pass; router imports from owlbear.config L26 | PASS |

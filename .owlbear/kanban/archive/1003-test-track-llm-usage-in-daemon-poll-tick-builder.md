...
1. Test that poll_tick passes tracker/model/provider to _run_builder_with_context in the retry-dispatch path (mock builder, assert record_agent_usage called with operation='builder_dispatch' and session_id='background:builder_dispatch' after awaiting result).
2. Test that poll_tick passes tracker/model/provider to _run_builder_with_context in the fresh-dispatch path (same assertions as AC1 but for new-task dispatch).

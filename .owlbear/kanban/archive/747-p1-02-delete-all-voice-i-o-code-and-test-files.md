...
### Tests
pytest: **18 passed, 0 failed** (tests/test_delete_voice_io_747.py)
...
- `serve/voice/**` → file_search: **no files found** ✓
- `serve/orchestrator/src/owlbear/voice/**` → file_search: **no files found** ✓
- `tests/test_voice_*.py` → file_search: only `test_voice_removal_clean_build_750.py` (unrelated) — all 6 target files **absent** ✓
- grep for voice imports in `serve/` → **0 matches** ✓

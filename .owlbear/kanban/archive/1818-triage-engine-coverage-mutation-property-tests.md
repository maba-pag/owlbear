...
updated: 2026-05-24T10:50:02.869407+02:00
tags:
  - scope:kanban-engine
  - test-failure
  - discussion
...
depends_on:
  - 1814
ac:
  - Edit-task mutation failures are classified separately from board-config
    deep-copy failures.
  - The current return contract for `edit_task` dependency/parent mutations is
    verified before changes.
  - Deep-copy expectations are checked against the current grouped `BoardConfig`
    model.
blocked: false
block_reason:
claimed_at:

...
depends_on: []
ac:
  - Root `tests/test_engine_*.py tests/test_kanban_*.py` failures are grouped
    into current, actionable causes.
  - Stale structural/regression tests are updated or retired only with explicit
    approval.
  - A rerun of the root glob has an understood pass/fail status with any
    remaining failures linked to specific tasks.
proof_bundle: smoke
blocked: false
block_reason:
claimed_at:

...
Write failing tests for #943 — extending mtime cache to show_task() and _find_task_path().
...
- [ ] Test: refresh_config() clears _id_to_filename dict
...
| refresh_config() clears _id_to_filename | 2 | COVERED |
...
| refresh_config() clears _id_to_filename | 2 | PASS |
...
| refresh_config() clears _id_to_filename | test_refresh_config_clears_id_to_filename, _reachable_after_re_warm | Yes — == {} comparison | COVERED |
...
- `show_task()` warm miss (task created post list_tasks; ID absent from _id_to_filename, cache populated for others) — code path is identical to cold-cache path; exercised indirectly. Low risk.
...
| refresh_config() clears _id_to_filename | test_refresh_config_clears_id_to_filename, _reachable_after_re_warm | Yes — == {} equality | COVERED |
...
- show_task() warm miss (ID absent from _id_to_filename after create): code path identical to cold-cache; indirectly exercised. Low risk.

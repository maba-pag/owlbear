...
- Architect response: (1) REBUTTED — save_config output already fails _is_config_migrated due to tasks_dir/archive_dir (both _LEGACY_CONFIG_KEYS, never stripped). Adding defaults/activity_log doesn't change the outcome. Migration and persistence are separate operations. (2) ACCEPTED — removed stale #1170 reference from AC4. (3) ACCEPTED — refined AC1 to explicitly specify denylist approach with dead keys.
...
| Test independence | STRONG | Each case provisions a fresh tmp_path board through _make_board() |

...
- `.owlbear/kanban/tasks/*.md` — 17 task files migrated (`status: ideation` → `status: research`)
...
- Task migration: all 17 tasks confirmed at `status: research`
...
|---------|----------|-------------|--------|
| config.yml renamed | `.owlbear/kanban/config.yml` L6: `name: research`; default L18: `status: research` | – | PASS |
| seed config renamed | `seed/.owlbear/kanban/config.yml` L6, L18: same | – | PASS |
| All ideation tasks migrated | grep `status: ideation` returns 0 task files | – | PASS |
...
| All ideation tasks migrated | grep "status: ideation" in .owlbear/kanban/tasks/: 0 matches (only historical refs in #641 body) | PASS |

...
| O7 | **Trustworthy foundation (Shared-tier)** | (a) Adapter test coverage at Shared bar. (b) Docs page lists engine surface used. (c) Induced engine failure produces visible recoverable error state within one poll interval. |
...
| HTTP routes (six v1 mutations + reads) | `GET /api/board` (config + valid_transitions), `GET /api/tasks` (TaskSummary list + mtime), `GET /api/tasks/{id}` (full Task + `updated` snapshot), `POST /api/tasks/{id}/move`, `POST /api/tasks/{id}/edit` (allowlisted YAML fields `title`, `tags`, `priority`, `depends_on`, `parent`, `block_reason` + body, requires `updated` snapshot for D9), `POST /api/tasks/{id}/release`, `GET /api/sessions?filter=` (derived from activity log). |
...
| Mtime-scan polling | Backend cache invalidates on tasks-dir mtime change (~1 ms scan); full reload only when files changed. |
...
| Polling | Mtime-aware short-poll @ 3 s. Skip 1 cycle after a local mutation. |
...
| Connection health | Status traffic light: green = poll < threshold, yellow = poll lagging, red = disconnected. |
...
- SSE / WebSocket (polling sufficient for 2–5 s staleness budget)
...
| R3 | Per-instance revision counter does **not** see cross-process writes | Mtime-scan polling instead (D14) — corrects the original research-notes assumption |
...
| R8 | Polling vs in-flight mutation race | Skip 1 poll cycle after local mutation; mtime check for change detection |
...
| D6 | Staleness budget: **2–5 s**; mtime-scan polling |

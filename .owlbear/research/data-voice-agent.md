# Data-Voice Agent Design

> **Owning task:** #648 — P4-08: Create data-voice.agent.md
> **Date:** 2026-04-06  **Status:** Complete

## 1. Context and Question

Task #648 creates the second domain voice subagent (`data-voice.agent.md`) for the Ideator's voice panel. It follows the architect-voice template pattern established by #647's research. Domain focus: data quality, validation, schemas, ETL pipeline patterns, data integrity, analytics.

**Key question:** Can data-voice reuse the architect-voice structural pattern with only persona/domain/file-path changes, or does the data domain require structural deviations?
...
| 2 | `.owlbear/research/architect-voice-agent.md` | Internal research | 0.95 |
| 3 | `share/agents/critic-voice.agent.md` | Codebase (built) | 0.90 |
...
### 3.1 Can data-voice reuse architect-voice pattern directly?

| Dimension | Architect-voice | Data-voice | Change needed? |
|-----------|----------------|------------|---------------|
| Frontmatter structure | T4, 8 tools, agents:[critic-voice] | Identical | No |
...
| Output files | voices/architect.md, voices/architect-debate.md | voices/data-person.md, voices/data-person-debate.md | File paths only |
...
### 3.2 Tool Set (inherits from architect-voice research)

**8 tools:** `[edit/createDirectory, edit/createFile, edit/editFiles, read/readFile, read/viewImage, search, vscode/memory, agent]`

Rationale unchanged from architect-voice analysis: file read/write for Working Dir, search for codebase verification, agent for Critic loop. No MCP tools (domain voices operate within Working Dir only).
...
|-------|-------|--------|
| name | data-voice | AC |
| description | Domain voice — data quality, validation, schemas, ETL, analytics | Spec §7 |
...
| disable-model-invocation | true | Critic-voice + architect-voice pattern |
| model | Claude Opus 4.6 (copilot) | AC + spec §12 |
| tools | 8-tool set (§3.2) | Architect-voice research §3.1 |
| agents | [critic-voice] | AC |
...
AC matches spec: `voices/data-person.md` and `voices/data-person-debate.md`.
...
| `test_disable_model_invocation.py` | Lists 8 specific pipeline agents — data-voice NOT in list | No conflict ✓ |

### 3.7 Estimated File Size

~65-75 lines (same as architect-voice estimate). Sections: frontmatter (~12), persona (~12), critical_rules (~8), Voice Reasoning Cycle (~15), Input/Output Contract (~15), boundaries/examples (~8).

### 3.8 Risks

| Risk | Severity | Mitigation |
|------|----------|-----------|
| Architect-voice not yet built when data-voice is built | Low | Research pattern is sufficiently detailed; both can be built independently from the same template |
| Persona too generic (not domain-specific enough) | Low | Use spec §7 quotes + data engineering domain knowledge in persona text |
| Tool set drift from architect-voice | Negligible | Both derive from same source (spec §12 read/write table) |

## 4. Recommendation

**Replicate architect-voice template with data domain adaptations** (confidence: 0.90)

Higher confidence than architect-voice (.88) because:
...
Implementation: ~65-75 line agent file. Persona encodes strong data quality/integrity opinions. Voice Reasoning Cycle with embedded Critic loop (≤5 cycles). Output to `voices/data-person.md` and `voices/data-person-debate.md`.
...
- (c) Is #647 dependency satisfied? #647 is in backlog (research done). Pattern is documented — data-voice can be built without waiting for architect-voice file to exist.
...
| 4 | "Agents list includes critic-voice" underspecified | Clarify: `agents: [critic-voice]` — sole subagent, not just "includes" |

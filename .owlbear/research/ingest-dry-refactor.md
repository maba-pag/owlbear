...
```powershell
kanban\kanban-md.exe create "Implement ingest() delegation to _ingest_from_intake()" --priority needed --tags "dry,refactor,knowledge,phase-10" --description "Refactor ingest() to call _ingest_from_intake() after intake+delta-check. Follow ingest_text() pattern. See docs/research/ingest-dry-refactor.md. AC: (1) ingest() delegates post-intake steps to _ingest_from_intake(). (2) No duplicated pipeline code between ingest() and _ingest_from_intake(). (3) All existing ingest tests pass. (4) Intake failure still produces a 'failed' IngestResult with document_id."
```

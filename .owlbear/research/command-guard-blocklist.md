...
```
kanban\kanban-md.exe create "Expand DEFAULT_BLOCKED_COMMANDS patterns to cover common bypass variants" --priority needed --tags "security,scope:core" --body "Expand from 6 to ~14 patterns per docs/research/command-guard-blocklist.md §3.4. Add: rm flag combos (-r -f, -fr, -Rf, --recursive --force), git push -f, python -m pip install, Remove-Item -Recurse -Force, find / -delete, mkfs., dd of=/dev/, git push --force-with-lease, del flag ordering. Update tests to cover new patterns + verify no false positives on safe commands. AC: all patterns from §3.4 implemented, tests green, no false positives on existing safe-command test list."

kanban\kanban-md.exe create "Add defense-in-depth documentation to command_guard.py module docstring" --priority needed --tags "security,docs,scope:core" --body "Add module-level docstring per docs/research/command-guard-blocklist.md §4.2. Document: blocklist is Layer 3 defense-in-depth not a security boundary, primary control is approval gate, known bypass categories (encoding, indirection, variables, aliasing, chaining), reference SEC-04. AC: docstring present, clearly states 'not a security boundary', lists bypass categories."
```

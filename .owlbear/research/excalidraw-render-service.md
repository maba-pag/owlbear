...
> **Date:** 2026-03-07 (re-evaluated 2026-03-11) **Status:** Complete

## 1. Context and Question

Task #629 proposes a Playwright-based `ExcalidrawRenderService` that renders `.excalidraw` JSON to PNG using OwlBear's existing `BrowserManager`. This is Tier 2 of the visuals strategy (see `docs/research/visuals-diagrams-mcp.md` §4). The key research questions:
...
|---|--------|-----|-----------|
| 1 | coleam00/excalidraw-diagram-skill | <https://github.com/coleam00/excalidraw-diagram-skill> | .95 — 1k+ stars, Playwright + esm.sh render pipeline (verified 2026-03-11) |
...
## 5. Re-evaluation (2026-03-11)

| Factor | 2026-03-07 | 2026-03-11 | Changed? |
|--------|-----------|-----------|----------|
| Kroki Excalidraw SVG-only | Yes | Yes (verified live) | No |
| coleam00 adoption | 693 stars | 1k+ stars, 187 forks | Growth validates approach |
| @excalidraw/excalidraw version | 0.18.0 | 0.18.0 (150k wkly downloads) | No |
| OwlBear Tier 0 implemented? | Not yet | **Still not** | Blocker: Tier 0 must come first |
| Concrete use case for PNG? | None | None | No |
| Task #628 (Excalidraw skill) | Proposed | In-progress (@test-writer) | Skill being built |

**Verdict: YAGNI still holds (.85 confidence).** No new evidence justifies building the Playwright service now. Downgrade #629 to `someday`, create the Tier 0 Kroki passthrough task at `backlog`.

## 6. Follow-up Tasks

```powershell
kanban\kanban-md.exe create "Add excalidraw to DiagramService SUPPORTED_TYPES (Kroki passthrough)" --priority needed --status backlog --tags "phase-7,scope:core,tooling" --body "One-line change: add 'excalidraw' to SUPPORTED_TYPES frozenset in src/owlbear/tools/diagram/service.py. Kroki already supports Excalidraw JSON as diagram_source. Output: SVG only (Kroki limitation). See docs/research/excalidraw-render-service.md section 4 Tier 0.`n`nAC:`n- [ ] SUPPORTED_TYPES includes 'excalidraw'`n- [ ] Test: generate(diagram_type='excalidraw', source=valid_json, output_format='svg') calls Kroki correctly`n- [ ] Existing tests still pass`n- [ ] ruff clean"

kanban\kanban-md.exe edit 629 --priority someday --body "DEFERRED: Kroki passthrough (Tier 0) covers Excalidraw rendering with zero new code. Build the Playwright-based ExcalidrawRenderService only when Kroki proves insufficient (font issues, PNG needed, offline). See docs/research/excalidraw-render-service.md section 3.4 for trigger conditions and section 3.5 for the proven coleam00 implementation pattern."
```

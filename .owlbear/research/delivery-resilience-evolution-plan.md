...
> **Implementation snapshot:** version-bound `put_design` and `admit_change`, `get_change`, unified request/block/disposition `answer`, version-bound `set_change_intent`, grouped `list_changes`, Builder `submit_result`, claim-facing `acquire_actions`, and high-level `repair` are implemented additively through core and MCP. The live **Post-baseline** census is **79 public core methods**, **58 registered MCP operations**, **261 root exports**, and **29 Cockpit Delivery routes**. The unfenced lifecycle MCP aliases, low-level answer/repair/publish/acquire MCP tools, and legacy publish-result MCP tool are retired; the constrained Repairer route is wired to Change-specific proposals and bounded Decision options. Action free-text provenance and remaining low-level retirement remain future gates.
>
> **Gate status:** Additive Phase 4 work and early Phase 5 caller cutover proceeded before Gate B and Gate C were proven. This is a recorded sequencing deviation, not a gate pass: background convergence has not received the required restart/two-host proof, coherent repair classification is incomplete, and Gate D and Gate E remain unmet. No public-surface retirement is authorized by this snapshot.
>
> **Authority-repair slice:** Snapshot schema 1 has an explicit one-version canonicalization path with predecessor identity evidence. Frontier schema 17 is accepted as a compatibility relabel to schema 18; it adds no new frontier fields or identity evidence. The core/MCP surface includes separate typed proposal and CAS repair operations for quarantined remote snapshots, plus a confirmation-gated repair for the known missing request-provenance defect and an explicit state-preserving admission carry-forward path. Focused tests prove these boundaries; the live B1/D1 artifacts remain untouched pending operator-controlled execution.

...
> **Implementation snapshot:** `get_change`, version-bound request `answer`, version-bound `set_change_intent`, grouped `list_changes`, Builder `submit_result`, and proposal-backed `repair_change` are implemented additively through core and MCP. The live **Post-baseline** census is **72 public core methods**, **57 registered MCP operations**, **252 root exports**, and **29 Cockpit Delivery routes**. The unfenced lifecycle MCP aliases are retired; the constrained Repairer route is wired to Change-specific proposals and bounded Decision options. Requestless-block/disposition parity, Action free-text provenance, and remaining low-level retirement remain future gates.

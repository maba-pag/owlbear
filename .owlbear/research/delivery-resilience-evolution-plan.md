...
> **Implementation snapshot:** `get_change`, version-bound request `answer`, version-bound `set_change_intent`, and proposal-backed `repair_change` are implemented additively through core and MCP. The live **Post-baseline** census is **70 public core methods**, **58 registered MCP operations**, **250 root exports**, and **29 Cockpit Delivery routes**. The constrained Repairer route is wired to Change-specific proposals and bounded Decision options; requestless-block/disposition parity, Action free-text provenance, and low-level retirement remain future gates.

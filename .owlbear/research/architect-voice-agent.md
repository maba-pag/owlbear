# Architect-Voice Agent Design

> **Owning task:** #647 — P4-07: Create architect-voice.agent.md
> **Date:** 2026-04-06 **Status:** Complete

## 1. Context and Question

Task #647 creates the first domain voice subagent (`architect-voice.agent.md`). It must follow the Voice Reasoning Cycle from the thinking companion spec (§7, §12): read context, form opinion from architectural lens, run embedded Critic loop (≤5 cycles), publish hardened position + debate log. Key constraint: this is the template for data-voice, enduser-voice, and security-voice — structure must be clean and generalizable.

**Questions investigated:**
1. What tool set does a domain voice need?
...
|---|--------|-----------|------|
| 1 | thinking-companion-framework.md §7, §12 | .95 | Voice panel architecture, Voice Reasoning Cycle, multi-model assignment, read/write table |
| 2 | share/agents/critic-voice.agent.md (#644) | .95 | First voice subagent built, structural pattern, dual-scope, read-only tools |
| 3 | share/agents/ideator.agent.md (#645) | .90 | Invoking agent — defines how domain voices are called, agents list, tool set |
...
| Tool | Critic-voice | Ideator | Architect-voice (rec) | Rationale |
|------|-------------|---------|----------------------|-----------|
| edit/createDirectory | — | Yes | Yes | Create voices/ dir |
...
| agent | — | Yes | Yes | Invoke critic-voice subagent |
...
Rationale: Read tools mirror Critic-voice (minus `read/problems` — not needed). Write tools added for file output. `agent` added for Critic loop. No MCP tools — domain voices operate only within the Working Dir + codebase read.

### 3.2 Structural Pattern (Options)

| Option | Description | Est. Lines | Score |
|--------|-------------|-----------|-------|
| A. Mirror critic-voice closely | Same sections: persona, critical_rules, Input/Output Contract | ~65-75 | .85 |
...
**Recommendation: Option A (.85)** — Mirror critic-voice with domain-specific adaptations. The Input Contract, Output Contract, and Voice Reasoning Cycle sections provide clear structure that future domain voices can replicate by changing only the persona and domain.
...
| Architect-Voice | architect-voice.agent.md | architect-voice | T4 Tools | Ideation domain opinion |

No conflict: different files, different names, different tiers, different dispatch chains. Pipeline architect is invoked by orchestrator; architect-voice is invoked by ideator. They never interact.

### 3.4 Frontmatter Decisions

| Field | Value | Source |
|-------|-------|--------|
| name | architect-voice | AC, distinct from pipeline `architect` |
| user-invocable | false | AC spec |
| disable-model-invocation | true | Standard for subagent-only T4 agents (critic-voice pattern) |
...
Build `share/agents/architect-voice.agent.md` as a ~65-75 line agent file following Option A (mirror critic-voice pattern with domain-specific adaptations):
...
- **Agents:** `[critic-voice]` — sole subagent
...
- **Body:** Voice Reasoning Cycle with embedded Critic loop (≤5 cycles), Input Contract (context.md, decisions.md, research-notes.md), Output Contract (voices/architect.md, voices/architect-debate.md)
- **Template role:** Structure must be cleanly replicable for data-voice, enduser-voice, security-voice (change persona + domain + file paths)

### AC Refinements for Architect Gate

1. **New AC:** `disable-model-invocation: true` set (standard for subagent-only agents; precedent: critic-voice #644)

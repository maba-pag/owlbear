...
```
kanban\kanban-md.exe create "Wrap channel.send in _recover_from_error with try/except fallback logging" --status todo --priority needed --tags "resilience,scope:core,phase-7" --body "Fix P-2 from resilience-audit.md. Wrap all 3 channel.send calls in _recover_from_error (TRANSIENT exhausted, AUTH failed, PERMANENT) in try/except. On failure: logger.exception() so the error is always persisted to the RotatingFileHandler. AC: (1) channel.send failure does not lose the original error, (2) fallback log entry contains the original exception, (3) daemon loop continues. See docs/research/recover-from-error-swallowing.md."

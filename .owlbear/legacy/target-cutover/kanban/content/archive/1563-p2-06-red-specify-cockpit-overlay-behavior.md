...
2026-05-15T06:07:29+00:00
## Review Evidence
- Verdict: FAIL
- FAIL confirmation: FAIL #1563 -> backlog | AC-2 tab-cycle proof is still too weak: the current dialog tests sample focus containment after fixed Tab counts instead of proving actual wrap/cycle behavior.
- Builder evidence review: Sufficient for review of this RED pass-through task. I accepted the latest task-body proof packet (19 tests total, 9 FAIL / 10 PASS, eslint clean) and inspected the scratch spec directly. No independent rerun was needed because the blocker is assertion strength, not missing execution evidence.
- Challenger cross-check: reconsider (0.71). Agreed on the AC-2 proof-sufficiency objection.
- Blocking findings:
| # | AC Line | Finding | Evidence | Route |
|---|---------|---------|----------|-------|
| 1 | AC-2 | The three task-local Tab-cycle tests prove only that focus is somewhere inside the dialog after a fixed number of Tab presses; they do not prove cycle/wrap between the dialog's focusable controls. A broken implementation that traps focus on the dialog root or keeps focus inside without wrapping between first/last controls could still pass. | `.owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:33`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:432`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:447`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:453`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:461`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:471`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:509`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:519`; `.owlbear/scratch/1563-overlay-behavior.spec.ts:530` | backlog |

### Required Follow-up
| # | Target Agent | Action Required | File(s) | Evidence |
|---|-------------|----------------|---------|----------|
| 1 | architect | Re-issue AC-2 proof instructions as explicit wrap/cycle assertions for each blocking dialog (for example: identify first and last focusable controls, assert Tab wraps last→first and Shift+Tab wraps first→last), then reroute to test-writer for test strengthening. | `.owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md`; `.owlbear/scratch/1563-overlay-behavior.spec.ts` | AC-2 at task:33 vs containment-only checks at scratch:447/471/530 and fixed Tab loops at scratch:461/519 |

## Observations
- I am not disputing the latest proof packet execution status: the current task body records 19 tests with 9 FAIL / 10 PASS and eslint clean, which is enough run evidence for review.
- AC-1 and AC-4 are no longer blocking. Cycle 7 resolved the earlier AC-1 coverage gaps and strengthened the context-menu checks into specific all-column, exact-count, and directional-navigation assertions.
- AC-3 still says "Test Evidence section" while the task records evidence under `## Test-Writer Notes`; I did not block on the naming mismatch, but it should be normalized on the next architectural rewrite to avoid future review ambiguity.
2026-05-15T06:18:39+00:00

## Architecture Review (Cycle 8 — Tab-Cycle Wrap Assertion Pattern)

### Context
Reviewer cycle 7 correctly identified that the three AC-2 Tab-cycle tests (ConfirmDialog, ResolveModal, ArchivalModal) prove only containment (`dialog.contains(activeElement)`) after fixed Tab counts, not actual wrap/cycle behavior. A broken implementation could false-green (e.g., focus stuck on dialog root, focus stays inside but never wraps between first/last). AC-2 literally says "Tab-cycle focus within the dialog" — "cycle" means wrap, not just containment.

### Codebase Evidence
- `ConfirmDialog.tsx:56-60`: only handles Escape — no Tab interception. Focusable elements: Cancel PButton, Confirm PButton (2 elements).
- `ResolveModal.tsx:122-127`: only handles Escape via document listener + modal onKeyDown. No Tab trap. Focusable elements: 3 radio inputs (`approved`/`rejected`/`needs-info`), PTextarea (`resolve-notes`), Submit PButton (`resolve-submit`), Close PButton (`resolve-cancel`) — ~6 elements.
- `ArchivalModal.tsx:140-170`: `handleKeyDown` implements Tab trap via `getFocusableElements()` with first→last / last→first wrapping. Focusable elements in non-error state: PSelect (reason), Archive PButton (`archival-submit`), Cancel PButton — 3 elements (conditionally PInputText for refs).

### Evaluation
| Criterion | Assessment | Notes |
|-----------|-----------|-------|
| Single responsibility | PASS | Unchanged |
| Interface clarity | PASS | AC-2 "Tab-cycle" already implies wrap; test assertion is the gap |
| Dependency correctness | PASS | Unchanged |
| TDD compliance | PASS | RED task; GREEN is #1569 |
| KISS/YAGNI | PASS | Unchanged |
| Pattern consistency | PASS | Directional wrap assertions are the standard a11y focus-trap pattern |

### AC Assessment
| AC Line | Assessment | Action |
|---------|-----------|--------|
| AC-1 | SATISFIED | Resolved in cycle 4 |
| AC-2 (Tab-cycle clause) | PASS — AC wording precise; test assertions must prove wrap not containment | Test-writer guidance below |
| AC-3 | SATISFIED | Resolved in cycle 4 |
| AC-4 | SATISFIED | Resolved in cycle 7 |

### Test-Writer Guidance (Cycle 8) — Tab-Cycle Wrap Assertions

Replace all three Tab-cycle tests' containment checks with directional wrap assertions using the following pattern:

**Required assertion pattern per dialog:**
1. Locate the first and last focusable elements within the dialog
2. Focus last element → press Tab → assert first element is focused (forward wrap)
3. Focus first element → press Shift+Tab → assert last element is focused (backward wrap)

**ConfirmDialog (RED — no Tab trap in source):**
- First focusable: Cancel PButton (`.filter({ hasText: 'Cancel' })`)
- Last focusable: Confirm PButton (second `p-button`)
- Forward wrap: focus Confirm → Tab → assert Cancel is focused → FAILS (Tab escapes dialog)
- Backward wrap: focus Cancel → Shift+Tab → assert Confirm is focused → FAILS (Shift+Tab escapes)

**ResolveModal (RED — no Tab trap in source):**
- First focusable: first radio input `[value="approved"]` inside `[data-testid="response-selector"]`
- Last focusable: Close PButton `[data-testid="resolve-cancel"]`
- Forward wrap: focus Close → Tab → assert approved radio is focused → FAILS (Tab escapes modal)
- Backward wrap: focus approved radio → Shift+Tab → assert Close is focused → FAILS (Shift+Tab escapes)

**ArchivalModal (GREEN guard — handleKeyDown traps Tab):**
- First focusable: PSelect `[name="archival-reason"]` (or first from `getFocusableElements()`)
- Last focusable: Cancel PButton (last `p-button` child)
- Forward wrap: focus Cancel → Tab → assert PSelect is focused → PASSES (handleKeyDown wraps last→first)
- Backward wrap: focus PSelect → Shift+Tab → assert Cancel is focused → PASSES (handleKeyDown wraps first→last)

**PDS Web Component note:** If PButton/PSelect shadow DOM interferes with `toBeFocused()`, use `page.evaluate()` to compare `document.activeElement` against the host element (or its closest testable ancestor). The test-writer may need to adapt locator strategy for PDS shadow DOM focus delegation.

### Challenge Results
- Challenger: SKIPPED — no AC or architecture changes; assertion-strength guidance only. Substance unchanged from cycle 1 challenger (which was addressed: 4/6 findings accepted).
...
### Verdict: APPROVE
### Action Taken: Issued explicit wrap/cycle assertion pattern replacing containment checks; per-dialog first/last element guidance with expected RED/GREEN status; advanced backlog → todo
2026-05-15T06:18:47+00:00
Cycle 8: Reviewer finding accepted — Tab-cycle tests used containment checks (dialog.contains(activeElement)) instead of proving wrap behavior. Issued explicit directional wrap assertion pattern: locate first/last focusable → Tab from last asserts first focused (forward wrap), Shift+Tab from first asserts last focused (backward wrap). Per-dialog guidance with concrete element locators: ConfirmDialog (Cancel/Confirm PButtons, RED), ResolveModal (approved radio/Close PButton, RED), ArchivalModal (PSelect/Cancel PButton, GREEN guard). No AC text changes needed.
2026-05-15T06:32:14+00:00
## Test-Writer Notes (Retry — Cycle 8)

**Gap addressed from Review Evidence (Cycle 7) — Finding 1:**
Replaced containment-check Tab-cycle tests with directional wrap assertions per architect cycle-8 guidance. All three tests now:
1. Focus the last focusable element → press Tab → assert first focusable is focused (forward wrap).
2. Focus the first focusable element → press Shift+Tab → assert last focusable is focused (backward wrap).

**Changes per dialog:**
- `confirm_dialog_tab_focus_cycles_within_dialog`: Cancel=nth(0), Confirm=nth(1); `confirmBtn.focus()→Tab→expect(cancelBtn).toBeFocused()` FAILS (Tab escapes dialog); `cancelBtn.focus()→Shift+Tab→expect(confirmBtn).toBeFocused()` FAILS.
- `resolve_modal_tab_focus_cycles_within_modal`: first=approved radio, last=`[data-testid="resolve-cancel"]`; `lastFocusable.focus()→Tab→approvedFocused via evaluate()` FAILS; `firstFocusable.focus()→Shift+Tab→expect(lastFocusable).toBeFocused()` FAILS.
- `archival_modal_tab_focus_cycles_within_modal`: first=PSelect `[name="archival-reason"]`, last=Cancel PButton; `cancelBtn.focus()→Tab→selectFocusedAfterTab via evaluate()` PASSES (handleKeyDown wraps); `archivalSelect.focus()→Shift+Tab→expect(cancelBtn).toBeFocused()` PASSES. GREEN guard confirmed.

**Test file:** `.owlbear/scratch/1563-overlay-behavior.spec.ts` (gitignored scratch)
**Builder copy instruction (#1569):** `cp .owlbear/scratch/1563-overlay-behavior.spec.ts serve/cockpit/web/e2e/overlay-behavior-1563.spec.ts && git add serve/cockpit/web/e2e/overlay-behavior-1563.spec.ts`

**Classes:** `TestFromAC_OverlayReflow`, `TestFromAC_BlockingDialogSemantics`, `TestFromAC_ContextMenuOverlay`
**Tests total: 19. 9 FAIL, 10 PASS. ESLint: clean (exit 0).**
...
| AC-2: role=dialog+aria-modal on ResolveModal | `resolve_modal_has_role_dialog_and_aria_modal` | PASS — source ResolveModal.tsx:157-158 |
| AC-2: role=dialog+aria-modal on ArchivalModal | `archival_modal_has_role_dialog_and_aria_modal` | PASS — source ArchivalModal.tsx:246 |
| AC-2: Tab wraps within ConfirmDialog (Cancel↔Confirm) | `confirm_dialog_tab_focus_cycles_within_dialog` | FAIL — Tab escapes dialog (no trap) |
| AC-2: Tab wraps within ResolveModal (approved radio↔Close) | `resolve_modal_tab_focus_cycles_within_modal` | FAIL — Tab escapes modal (no trap) |
| AC-2: Tab wraps within ArchivalModal (PSelect↔Cancel) | `archival_modal_tab_focus_cycles_within_modal` | PASS (green guard) — handleKeyDown wraps |
| AC-2: Focus returned after ResolveModal close | `resolve_modal_focus_returned_to_trigger_after_close` | FAIL — no previousFocusRef (exact dr-item-* assertion) |
| AC-2: Focus returned after ArchivalModal close | `archival_modal_focus_returned_to_trigger_after_close` | FAIL — no previousFocusRef |
| AC-2: Focus returned after ConfirmDialog close | `confirm_dialog_focus_returned_to_trigger_after_close` | PASS (green guard) — previousFocusRef restores trigger |
| AC-4: All column heights unchanged on context menu open | `context_menu_renders_as_fixed_overlay_without_column_reflow` | PASS — position:fixed (all columns measured) |
| AC-4: role=menu + exact 3 menuitem children | `context_menu_has_role_menu_and_menuitem_children` | PASS — 3 transitions for in-progress task |
| AC-4: ArrowDown moves to nth(1), ArrowUp returns to nth(0) | `context_menu_keyboard_navigation_with_arrow_keys` | PASS — directional movement verified |
| AC-4: Focus returned to card after context menu Escape | `context_menu_focus_returned_to_card_after_escape` | FAIL — no focus restore |
...
**Verification:** quality-runner scoped Playwright run (chromium, workers=1) — 9 failed, 10 passed, ESLint exit 0.
2026-05-15T06:42:24+00:00
## Builder Notes
- Task classification: RED specification task tagged `type:test`; AC-1..AC-4 are test-writer directives and implementation remains scoped to #1569.
- Pass-through trigger: latest Test-Writer Notes (Retry — Cycle 8) and prior architecture routing explicitly mark #1563 as builder non-implementation handoff.
- Files changed by builder: none.
- Tests run by builder: none (non-implementation pass-through task).
- Coverage/lint by builder: not applicable (no code changes).
- Evidence summary: task-local RED artifact currently reports 19 total E2E checks with 9 failing / 10 passing and ESLint clean; this is the expected failing-proof bundle for downstream GREEN implementation in #1569.
- Fixes applied: none in this task.

2026-05-15T07:15:50+00:00
## Review Evidence
- Verdict: FAIL
- FAIL confirmation: task 1563 routes to backlog because AC-2 archival focus-return proof still does not prove return to the triggering element.
- Builder evidence review: Sufficient for review of this RED pass-through task. I used the latest task-body proof packet at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:753-779 (19 total tests, 9 FAIL, 10 PASS, ESLint clean) plus direct scratch and source inspection. No independent rerun was needed because the blocker is proof-contract shape, not missing execution evidence.
- Challenger cross-check: reconsider (0.74). Agreed. The remaining gap is the archival focus-return target and assertion strength.
- Blocking findings:
| # | AC Line | Finding | Evidence | Route |
|---|---------|---------|----------|-------|
| 1 | AC-2 | `archival_modal_focus_returned_to_trigger_after_close` still does not prove return to the triggering element required by AC-2. The modal is opened by clicking the archived context-menu item, but the test pre-focuses the card, treats the card as the trigger, and only asserts `card.contains(document.activeElement)` rather than exact focus on a clarified target. That can false-green both if the intended trigger is the menuitem and if the intended return target is the card host itself. | .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:33; .owlbear/scratch/1563-overlay-behavior.spec.ts:624-625; .owlbear/scratch/1563-overlay-behavior.spec.ts:633; .owlbear/scratch/1563-overlay-behavior.spec.ts:650; .owlbear/scratch/1563-overlay-behavior.spec.ts:654; serve/cockpit/web/src/KanbanBoard.tsx:201-205; serve/cockpit/web/src/KanbanBoard.tsx:368; serve/cockpit/web/src/KanbanBoard.tsx:397; serve/cockpit/web/src/components/Card.tsx:83; serve/cockpit/web/src/components/Card.tsx:89-90 | backlog |

### Required Follow-up
| # | Target Agent | Action Required | File(s) | Evidence |
|---|-------------|----------------|---------|----------|
| 1 | architect | Clarify the ArchivalModal AC-2 focus-return target after close (archived menuitem vs originating card) and update the task body so the required target is explicit despite the context menu being torn down when `archived` is chosen. | .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md; serve/cockpit/web/src/KanbanBoard.tsx | AC-2 at task:33 vs modal-open path at scratch:633 and KanbanBoard.tsx:201-205, 368, 397 |
| 2 | architect | Re-issue test guidance so `archival_modal_focus_returned_to_trigger_after_close` asserts exact focus on the clarified target, not broader containment. If the target is the card, require exact host focus; if the target is the menuitem, specify the required menu lifecycle. | .owlbear/scratch/1563-overlay-behavior.spec.ts; .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md | Broad assertion at scratch:624-625, 650, 654 vs exact-trigger AC wording at task:33 and exact-card host evidence at Card.tsx:83, 89-90 |

## Observations
- AC-1 and AC-4 are no longer blocking. The current RED file covers all six AC-1 surfaces at .owlbear/scratch/1563-overlay-behavior.spec.ts:223, 240, 257, 275, 303, 328 and the AC-4 task-local proof now covers all-column reflow, exact 3-item menu structure, directional ArrowDown and ArrowUp navigation, and Escape-close focus return at .owlbear/scratch/1563-overlay-behavior.spec.ts:687, 719, 740, 767.
- AC-2 is otherwise materially stronger now: aria checks exist for all three dialogs at .owlbear/scratch/1563-overlay-behavior.spec.ts:390, 401, 414; Tab-cycle assertions now use wrap checks rather than containment at .owlbear/scratch/1563-overlay-behavior.spec.ts:434, 469, 544; and ConfirmDialog plus ResolveModal focus-return checks now aim at exact targets at .owlbear/scratch/1563-overlay-behavior.spec.ts:509, 590.
- Route is backlog because this task has already looped through review and the remaining defect sits at the AC/proof boundary rather than straightforward builder work.
2026-05-15T07:17:50+00:00
## Architecture Review (Cycle 9 — ArchivalModal Focus-Return Target Clarification)

### Context
Reviewer cycle 8 correctly identified that `archival_modal_focus_returned_to_trigger_after_close` uses a broad `card.contains(document.activeElement)` check instead of exact-element focus. The gap: AC-2 says "return focus to the triggering element after close" but the test doesn't clarify what "triggering element" means when the original menuitem is torn down.

### Codebase Evidence (focus-return target determination)
- `KanbanBoard.tsx:202`: `setContextMenu(null)` tears down the context menu DOM before `setArchivalModal(...)` runs.
- `KanbanBoard.tsx:134-140`: `handleContextMenu` stores `taskId` in the context menu state.
- `Card.tsx:83-90`: Card host element has `role="button"`, `tabIndex={0}`, `data-testid="task-card"`, `data-id={task.id}` — it is focusable.
- ARIA APG focus-return guidance: when the triggering element is removed from DOM, return focus to the nearest logical ancestor that initiated the interaction.

### AC-2 Focus-Return Target Clarification
**ArchivalModal trigger chain:** Card (right-click) → context menu opens → "archived" menuitem clicked → context menu removed from DOM → ArchivalModal opens → modal closes → ???

**Resolution:** The triggering element for ArchivalModal is the **originating card host element** (`[data-testid="task-card"][data-id="${taskId}"]`). Rationale:
1. The menuitem that directly triggered the modal is torn down (`setContextMenu(null)`) before the modal opens — it cannot receive focus.
2. The card is the nearest focusable ancestor that initiated the entire interaction chain (card right-click → menu → modal).
3. This matches ARIA APG guidance for removed triggers.
4. The card element has `role="button"` + `tabIndex={0}` — it is a valid focus target.

### Test-Writer Guidance (Cycle 9)
Replace `card.contains(document.activeElement)` containment check in `archival_modal_focus_returned_to_trigger_after_close` with exact-element equality:
```
const cardEl = page.locator(`[data-testid="task-card"][data-id="${taskId}"]`)
// ... open context menu, click archived, wait for modal, close modal ...
await expect(cardEl).toBeFocused()
```

This is a RED assertion because the current implementation has no `previousFocusRef` tracking in ArchivalModal (confirmed in prior cycles). The GREEN implementation in #1569 will need to store the originating card reference before opening the modal and restore focus to it on close.

### Evaluation
| Criterion | Assessment | Notes |
|-----------|-----------|-------|
| Single responsibility | PASS | Unchanged |
| Interface clarity | PASS | AC-2 "triggering element" now clarified for ArchivalModal's torn-down-trigger case |
| Dependency correctness | PASS | Unchanged |
| TDD compliance | PASS | RED task; GREEN is #1569 |
| KISS/YAGNI | PASS | Unchanged |
| Pattern consistency | PASS | Card-as-fallback-target matches standard ARIA APG removed-trigger pattern |

### AC Assessment
| AC Line | Assessment | Action |
|---------|-----------|--------|
| AC-1 | SATISFIED | Resolved in cycle 4 |
| AC-2 (focus-return: ArchivalModal) | PASS — target clarified as originating card element; exact-element assertion required | Test-writer tightens one assertion |
| AC-3 | SATISFIED | Resolved in cycle 4 |
| AC-4 | SATISFIED | Resolved in cycle 7 |

### Challenge Results
- Challenger: SKIPPED — no AC or architecture changes; single assertion target clarification only. Focus-return target is architecturally determined by DOM lifecycle (menuitem doesn't exist after setContextMenu(null)), not by design choice.
...
### Verdict: APPROVE
### Action Taken: Clarified ArchivalModal AC-2 focus-return target as originating card element (menuitem torn down before modal opens per KanbanBoard.tsx:202); issued exact-element assertion guidance; advanced backlog → todo
2026-05-15T07:17:57+00:00
Cycle 9: Reviewer finding accepted — ArchivalModal focus-return assertion used broad containment (card.contains(activeElement)) instead of exact-element focus. Clarified: the AC-2 "triggering element" for ArchivalModal is the originating card host element ([data-testid="task-card"][data-id="${taskId}"]) because the menuitem is torn down via setContextMenu(null) at KanbanBoard.tsx:202 before the modal opens. Issued exact-element assertion guidance (await expect(cardEl).toBeFocused()). No AC text changes needed — wording is already precise; test assertion is the gap. Re-approved to todo.
2026-05-15T07:47:07+00:00
## Test-Writer Notes (Retry — Cycle 9)

**Gap addressed from Review Evidence (Cycle 8) — Finding 1:**
Tightened `archival_modal_focus_returned_to_trigger_after_close` from broad containment check (`card.contains(document.activeElement)`) to exact-element equality using Playwright's `expect(cardEl).toBeFocused()`.

**Exact change made:**
- Replaced `page.evaluate(() => cardEl.contains(document.activeElement))` + `.toBe(true)` with `page.locator('[data-testid="task-card"][data-id="1"]')` + `await expect(cardEl).toBeFocused()`
- Rationale (per cycle 9 architect guidance): The archived menuitem is torn down via `setContextMenu(null)` at `KanbanBoard.tsx:202` before ArchivalModal opens — the originating card element is the correct ARIA APG fallback focus target. The card host has `role="button"` + `tabIndex={0}`, is always in DOM, and `data-id="1"` uniquely identifies the triggering card.
- Still FAILS: ArchivalModal has no `previousFocusRef` or equivalent focus-storage/restore mechanism.
...
**AC Coverage (unchanged from cycle 8 except assertion tightened):**
...
| AC-2: Tab wraps within ArchivalModal (PSelect↔Cancel) | `archival_modal_tab_focus_cycles_within_modal` | PASS (green guard) — handleKeyDown wraps |
| AC-2: Focus returned after ResolveModal close | `resolve_modal_focus_returned_to_trigger_after_close` | FAIL — no previousFocusRef (exact dr-item-* assertion) |
| AC-2: Focus returned after ArchivalModal close | `archival_modal_focus_returned_to_trigger_after_close` | FAIL — no previousFocusRef (exact card element assertion) |
| AC-2: Focus returned after ConfirmDialog close | `confirm_dialog_focus_returned_to_trigger_after_close` | PASS (green guard) — previousFocusRef restores trigger |
| AC-4: All column heights unchanged on context menu open | `context_menu_renders_as_fixed_overlay_without_column_reflow` | PASS — position:fixed (all columns measured) |
| AC-4: role=menu + exact 3 menuitem children | `context_menu_has_role_menu_and_menuitem_children` | PASS — 3 transitions for in-progress task |
| AC-4: ArrowDown moves to nth(1), ArrowUp returns to nth(0) | `context_menu_keyboard_navigation_with_arrow_keys` | PASS — directional movement verified |
...
**AC-2 per-surface proof matrix (complete):**
...
**Verification:** quality-runner scoped Playwright run (chromium, workers=1) — 9 failed, 10 passed (19 total confirmed by grep), ESLint exit 0.
2026-05-15T07:48:42+00:00
## Builder Notes
- Task classification: RED specification task tagged `type:test`; acceptance criteria are test-writer directives and implementation remains scoped to #1569.
- Pass-through basis: architecture routing cycles 3/5/6/7/8/9 explicitly mark #1563 as builder non-implementation handoff.
- Files changed: none.
- Tests run by builder: none (no builder-owned implementation contract in this task).
- Lint run by builder: none (no file changes).
- Coverage: not applicable (no source changes).
- Evidence summary: latest Test-Writer retry (cycle 9) records a task-local RED artifact with 19 total E2E checks (9 failing, 10 passing), plus ESLint clean via quality-runner. That is the expected failing-proof bundle for downstream GREEN implementation in #1569.
- Fixes applied: none in this task (pass-through).
2026-05-15T08:25:22+00:00
## Review Evidence
- Verdict: PASS
- PASS confirmation: PASS #1563 -> docs | AC mapped to code and evidence sufficient.
- Builder evidence review: Sufficient for this RED pass-through task. The latest task-body packet records 19 named E2E checks with 9 expected RED failures, 10 green-guard passes, and ESLint clean at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:890, .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:916, and .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:924.
- Challenger cross-check: reconsider (0.74). I overrode that challenge only after an independent quality-runner Playwright rerun reproduced `dr_indicator_popover_does_not_expand_status_bar` as FAIL (`Expected: 57, Received: 133`) and `resolve_modal_does_not_expand_shell_or_status_bar` as PASS. The static source read was ambiguous on the intermediary DR popover path; the runtime proof packet resolved that ambiguity.
- AC evidence map:
| AC Line | Code Evidence | Test Evidence | Status |
|---|---|---|---|
| AC-1 | Status bar is an in-flow flex header in serve/cockpit/web/src/Shell.css:25. DR item trigger remains directly addressable in serve/cockpit/web/src/components/DRStatusIndicator.tsx:84-86. | Six desktop reflow checks in .owlbear/scratch/1563-overlay-behavior.spec.ts:223, :240, :257, :275, :303, and :328, with current packet rows at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:896-901. Independent quality-runner rerun also confirmed the questioned DR-indicator / ResolveModal pair. | PASS |
| AC-2 | ConfirmDialog captures/restores previous focus and exposes aria-modal in serve/cockpit/web/src/components/ConfirmDialog.tsx:45-49 and :65. ResolveModal exposes aria-modal and only handles Escape / initial focus in serve/cockpit/web/src/components/ResolveModal.tsx:98, :110-123, :158, and :233. ArchivalModal traps Tab and exposes aria-modal in serve/cockpit/web/src/components/ArchivalModal.tsx:103, :146, :159, :165, and :246. Card host is the exact fallback focus target in serve/cockpit/web/src/components/Card.tsx:83, :89, and :90. | Nine task-local dialog-semantics checks in .owlbear/scratch/1563-overlay-behavior.spec.ts:390, :401, :414, :434, :469, :509, :544, :590, and :620, plus the complete per-surface matrix at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:916-921. | PASS |
| AC-3 | Latest Test-Writer Notes explicitly record the failing inline-expansion and missing-focus-management evidence in the current AC coverage block. | Current evidence block at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:892-924 maps those failures to named tests and quality-runner verification. | PASS |
| AC-4 | KanbanBoard closes the context menu with `setContextMenu(null)`, focuses the first menuitem on open, and renders `role="menu"` / `role="menuitem"` with arrow-key handlers in serve/cockpit/web/src/KanbanBoard.tsx:103, :107, :113, :359, :368, and :379. | Four context-menu checks in .owlbear/scratch/1563-overlay-behavior.spec.ts:687, :719, :740, and :767, with current packet rows at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:911-914. | PASS |
- Blocking findings: none.

## Observations
- .owlbear/scratch/1563-overlay-behavior.spec.ts:19-29 still says some AC-2 / AC-4 green clauses are documented-only, but task-local tests now exist at .owlbear/scratch/1563-overlay-behavior.spec.ts:390, :401, :414, :719, and :740. Clean up that header when #1569 copies the file.
- AC-3 still says "Test Evidence section" at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:34 while the live evidence resides under Test-Writer Notes / AC Coverage at .owlbear/kanban/tasks/1563-p2-06-red-specify-cockpit-overlay-behavior.md:892-924. I did not treat that heading mismatch as blocking because the current evidence block is explicit and complete.
2026-05-15T08:27:18+00:00
## Docs Gate
### Checklist
| # | Check | Applies? | Status | Evidence |
|---|-------|----------|--------|----------|
| 1 | README verification | No | N/A | Zero source files changed across all pipeline cycles (test-writer and builder notes uniformly record "files changed: none"). Convention mapping yields no `serve/{pkg}/README.md` target. |
| 2 | External attribution | No | N/A | Implementation drew exclusively on existing Cockpit source, AC text, and standard ARIA/Playwright patterns. No third-party sources cited. |
| 3 | Research doc | No | N/A | Task body references pre-existing research docs (cockpit-visual-audit-consolidated-2026-05-14.md, 1560-cockpit-design-policy.md) as upstream inputs. No new research artifact was produced by this task. |
| 4 | Deletion detection | No | N/A | No files deleted. Builder and test-writer notes confirm zero file changes across all nine cycles. |

### Verification Layers
- Layer 1 — No source symbol, CLI flag, or API surface changed; grep for README updates not applicable.
- Layer 2 — LLM editorial: task is a RED test-specification pass-through; all deliverable content lives in `.owlbear/scratch/1563-overlay-behavior.spec.ts` (gitignored). No public-facing documentation was touched or made inaccurate.

### Files Updated
- None

### Scratch Files Cleaned
- Deleted: `1563-eslint-output.txt`, `1563-playwright-output.txt` (temp evidence captures)
- Preserved: `1563-overlay-behavior.spec.ts` — cross-task handoff artifact required by #1569 builder (`cp .owlbear/scratch/1563-overlay-behavior.spec.ts serve/cockpit/web/e2e/overlay-behavior-1563.spec.ts`). Deleting this here would break the downstream GREEN build task.
2026-05-15T08:49:57+00:00
## Audit
### Regression Detection
- quality-runner mode full: 4544 passed, 222+ failed (all from other tasks' RED specs and pre-existing debt), lint clean (ruff exit 0)
- Task #1563 changed ZERO source files — no regressions attributable to this task
- regression verdict: PASS

### Intent Verification
- scope alignment: PASS (scratch E2E spec targets cockpit overlay surfaces exactly as AC describes; domain tags match)
- purpose match: PASS (RED specification deliverable with 19 named E2E tests documenting overlay behavior gaps for downstream GREEN #1569)
- extraneous scope: none
- boundary check: function-level behavior verification deferred to reviewer

### Architect Quality: 3/5
AC lines were specific in naming components, viewport constraints, ARIA attributes, and verification methods. However, the proof-contract model (carry-forward vs task-local checks for already-green behaviors) was ambiguous from cycle 1 and caused ~5 unnecessary review cycles before being resolved. Assertion-strength expectations and exact focus-return targets were clarified only after repeated reviewer findings. The 9-cycle pipeline traversal is excessive for a RED spec task — much of it was preventable with upfront proof-contract clarity.

### Commit Integrity
- upstream commit presence: PASS (N/A — task produces no source commits; deliverable is gitignored scratch file `.owlbear/scratch/1563-overlay-behavior.spec.ts` directed to #1569 builder for copy+commit. Verified present on disk.)
- kanban commit packaging: pending (this audit cycle)

### Deduction Breakdown
- AC quality score 3 ≤ 3: -.03
- No other deductions (no regressions, no intent mismatch, no evidence gaps, clean lint, reviewer evidence section present)

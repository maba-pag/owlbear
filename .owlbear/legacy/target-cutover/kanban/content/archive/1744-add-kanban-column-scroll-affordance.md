...
ac:
  - Use the
  - Show a subtle continuation affordance when a Kanban column has more vertical
    content below its visible body.
  - Hide the affordance when the column is scrolled to the bottom or has no
    vertical overflow.
  - Preserve the bounded board layout and horizontal column strip behavior.
  - Add or update focused tests for the column scroll-affordance contract where
    practical.

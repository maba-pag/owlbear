...
updated: 2026-05-24T10:50:02.801746+02:00
tags:
  - scope:kanban-engine
  - test-failure
  - discussion
...
depends_on: []
ac:
  - The default `list_sessions` filter returns running and stuck sessions from
    valid log lines.
  - Invalid JSON activity-log lines are skipped without dropping later valid
    session entries.
  - '`serve/kanban/tests/test_list_sessions.py::TestFromAC_ListSessions::test_default_filter_returns_running_and_stuck_only`
    passes.'
  - The `TestFromAC_EngineListSessions` coverage tests for active sessions and
    invalid JSON pass.
proof_bundle: smoke
blocked: false
block_reason:
